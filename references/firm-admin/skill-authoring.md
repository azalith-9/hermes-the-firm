...
**`description`** — this is where most skill authors underinvest. the agent uses this to decide whether to auto-invoke your skill when the user says something. A vague description means the skill won't get invoked when it should. A precise one means it will.
...
Open with one paragraph telling the agent who it is in this skill and what the goal is.
...
Tell the agent what to expect as input and what to assume.
...
This is the bulk of the skill. Walk through what the agent should do, step by step. Number the steps. Be specific.
...
You don't have to write a skill from scratch. The fastest path is to let the agent write the first draft.

The procedure:

1. Open Hermes session. Create a project for skill authoring.
...
3. Ask the agent: *"Read these examples. Identify the pattern. Write a skill in the standard four-part format (role, inputs, procedure, output) that would automate this work. Use the standard verification boilerplate at the bottom."*
4. the agent produces a 90% draft.
...
Robert Graham's question — *"How do you suggest feeding the agent skills that incorporate our voice, tone, and position?"* — is the customization question.
...
the agent will dispatch correctly when the skill name matches an installed skill.
