...
This is the customization ritual. It is the single highest-leverage thing a legal team can do when adopting the agent. Skipping it produces the off-the-rack-suit experience the early plugin shipped with.
...
The voice section is what makes the agent's drafts feel like your drafts. Mark used the example of being a former Slack product counsel: the agent knows his emoji preferences when posting to deal channels.
...
A markdown file on your local machine — typically `practice-profile.md` or similar, named per the plugin. The file is loaded into every session running that plugin. It's plain text. You can edit it directly any time, or you can ask the agent to update it (*"update my practice profile — we just rolled out a new vendor MSA template"*).
...
Treat this file the way you treat a PRACTICE.md or a senior associate's "how I like things done" doc. It's living documentation; it improves with use.
...
### "The voice still sounds like the agent, not me"
...
The update is conversational: *"My practice profile says we use BoxCorp's NDA template, but we switched to Ironclad's standard last month. Update."* the agent rewrites the file.

---

## The deeper point

The cold-start interview is the move that turns the agent from a smart tool into a coworker who knows your job. Harry's framing — *"the agent is your coworker, not just a tool that you're using or a chatbot"* — only works if you've done the upfront customization.
...
The legal teams that get the most out of the agent are not the ones who buy the most expensive tier or wire the most connectors. They are the ones who spend fifteen minutes upfront answering the interview honestly, and then maintain the practice profile like they'd maintain any other piece of firm infrastructure.

---

## Source

Interview structure and quotes from the *How Legal Teams Put the agent to Work* webinar (Anthropic, May 2026; Mark Pike and Harry from Applied AI). Failure modes and the "Quick first, Full before production" recommendation are HAQQ deployment patterns, not direct webinar content.
