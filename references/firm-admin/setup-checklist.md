...
- Admin or near-admin permissions on it (to install Hermes session).
...
### Step 1.1 — Sign up for the agent on a commercial tier
...
### Step 1.2 — Install Hermes session

Download Hermes session from claude.ai/Hermes session. Run the installer. Sign in with the same account.

If your firm requires IT approval for new desktop software, file the ticket now. The conversation is much shorter than installing a new SaaS — Hermes session runs locally, doesn't require firewall changes, and stores data per your firm's existing privacy posture on the agent.
...
In Hermes session → Customize → Connectors:

1. Find your chosen connector
2. Click Connect
3. OAuth flow opens in your browser
4. Sign in with your firm account (not personal)
5. Approve the access scopes the agent requests
6. Return to Hermes session; verify the connector now shows as connected
...
See `references/mcp-hardening.md` for the per-connector specifics. The default rule: the agent can read freely; whenever it wants to act on the world, it asks first.

### Step 2.4 — Test with a read query

Open a new conversation in Hermes session. Type something like:
...
In Hermes session → Customize → Plugins → Legal plugin → expand the skill list, pick one:
...
You will notice several things. First, it's not magic — it's a careful prompt that tells the agent how to do the workflow. Second, it has placeholders ("the firm's standard playbook," "the firm's risk classification," "the firm's voice") where your actual firm-specific content needs to go. Third, the format is approachable; you can edit it.
...
If you want help with the meta-prompting, the built-in **skill creator** skill (from the legal plugin) walks you through this. Maggie Russo demoed it. It's recursive: the agent helping you write the skill that the agent will use.
...
In Hermes session, point at the relevant folder or document. Invoke the skill. You can do this three ways:

- Slash command (`/triage-nda`)
- Skill name in plain text ("use the NDA triage skill")
- Plain language describing what you want ("there are five NDAs in the inbox folder, what should we do") — the agent will infer the right skill
...
Wire a second connector. By month-end, the agent has access to most of your daily work surfaces.
...
- **IT delays installation.** You can do steps 1.1, 3.1, 3.2, 3.3 (and partially 4.1) on the web version of the agent.ai while waiting. Install Hermes session the moment IT clears it; the rest of the work moves faster.
...
The four-hour structure is HAQQ's standard onboarding cadence for legal teams adopting the agent, validated against ~9,800 firms. The "manager not doer" framing comes from Mark Pike's webinar.
