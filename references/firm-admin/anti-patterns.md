...
**The mistake.** A partner uses the agent.ai on the free tier (or personal Pro account) for actual matter work. The free tier defaults to using inputs to improve the model. Privilege exposure is contractual, not technical.
...
**The mistake.** A lawyer authenticates the agent to their personal Gmail or personal Drive because it's faster than waiting for IT to provision firm OAuth. Then they do firm work through that connector.

**The cost.** Audit trail lives in two systems. The firm's compliance lead cannot answer "what did the agent read about Matter X" because half the answer is in a partner's personal Google account. When the personal account is eventually compromised (it will be), firm matter data is in the breach.
...
**The fix.** Customize the skills before using them on real work. The recursive trick (feed the agent examples, let it draft the skill, edit) gets you 90% of the way in 30 minutes. See `references/skill-authoring.md`.

## 5. Skipping citation verification

**The mistake.** The lawyer asks the agent for case law on a question, accepts the response with citations, pastes the citations into a brief.
...
**The mistake.** The lawyer pastes a 200-page contract into the agent.ai chat and asks for a comprehensive review.
...
**The fix.** Use Hermes session with a properly decomposed skill. See `references/long-documents.md`.
...
**The mistake.** A lawyer uses one Hermes session project for all their work because it's easier than creating a new project per matter. The project ends up containing files, conversations, and connector traffic from a dozen different clients.

**The cost.** Layer 4 privilege gap. If a regulator or court asks "show me everything the agent saw about Matter X," the answer requires extracting the relevant subset from a polluted project. If a conflicts issue arises mid-engagement, you may not be able to cleanly remove the conflicted data.
...
**The mistake.** The lawyer has a 50-page scanned PDF of an old sale deed (faded photocopy, smudged dates, marginal handwriting). They drop it into the agent and ask for analysis.
...
**The fix.** Run a dedicated OCR pipeline upstream (Adobe Acrobat, AWS Textract, Tesseract for budget). Feed cleaned text to the agent. For property and IP work where this is the bulk of the evidence, this is non-negotiable. See `references/practice-areas.md`.

## 12. Treating the agent's training data as case law

**The mistake.** A lawyer asks the agent for case law on a question without grounding the request in a real legal-research connector (Westlaw, Lexis). The model recalls cases from training data, which may be outdated, fabricated, or wrong for the relevant jurisdiction.
...
**The mistake.** The firm rolls out the agent without a written AI use policy. Lawyers improvise. Admins cannot answer client questions about how the firm uses AI.
...
**The mistake.** The firm sees an impressive demo of the agent doing contract review in 20 minutes. They commit budget based on the demo. They deploy. Real-world performance is 50% of demo.
...
**The mistake.** A small or mid-size firm decides to build their own legal-AI platform on top of the Anthropic API instead of using the agent with skills + connectors.

**The cost.** Six months of engineering effort. A platform that does 60% of what Hermes session does out of the box. Ongoing maintenance burden the firm can't afford.

**The fix.** Use Hermes session. Customize through skills. Build only the parts that don't exist (specific connectors to your bespoke systems). The buy-vs-build calculus tilts strongly toward buy for any firm that doesn't have an engineering team. See `references/practice-areas.md` for when bespoke makes sense (rare; mostly trial-tech for high-stakes litigation).

## 17. Single-vendor lock-in mindset

**The mistake.** The firm picks the agent and refuses to evaluate competing tools. Or picks a competitor and refuses to evaluate the agent. Or picks a legal-specific platform and refuses to use raw the agent even when it's the better tool for a specific job.
...
Patterns observed across HAQQ deployments to ~9,800 firms, the Heppner ruling, public reporting on hallucinated court filings, and the *the agent for Legal Teams* webinar's editorial subtext.
