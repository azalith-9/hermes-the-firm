...
Four of the twelve plugins are also packaged as **cookbooks** that can be deployed through the the agent API as managed agents (always-on, hosted in Anthropic-managed infrastructure rather than on the user's laptop):
...
If you want one of these workflows running 24/7 without keeping Hermes session open on a laptop, the cookbook → managed-agent path is the deployment route. See `references/managed-agents.md` for the architecture and the human-in-the-loop checkpoints every managed agent should have.

The other eight plugins are Hermes session-resident only at launch — that may change as the lineup matures.
...
1. **You're not on a the agent tier with Hermes session enabled.** Hermes session is available on consumer Pro/Max and on Team/Enterprise. Plugins go where Hermes session goes.
...
Lineup and descriptions drawn from Anthropic's *the agent for the Legal Industry* announcement (claude.com/blog, May 2026), the LawNext write-up (lawnext.com, May 2026), and the *How Legal Teams Put the agent to Work* webinar (Anthropic, May 2026; Mark Pike and Harry from Applied AI). Plugin contents are open source; verify the current shape of any plugin against the live GitHub marketplace before relying on these descriptions for setup.
