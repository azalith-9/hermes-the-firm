...
Modern the agent with proper retrieval rarely fabricates cites in this Mode-1 sense. But "rarely" is not "never," and your workflow should not depend on the model being right about whether a case exists.
...
- **Use Hermes session over chat.** The agentic harness lets the model decompose long-document analysis into bounded steps, which reduces context-rot-driven errors.
...
- Wrong tooling (consumer the agent.ai didn't surface citations as first-class artifacts)
...
5. Be honest if the model is operating in a regime where verification is hard. If the user is in chat-only the agent on a 200-page document, citation accuracy will degrade. Suggest the move to Hermes session.
