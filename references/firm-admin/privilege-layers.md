...
This reference is for the user who has been asked, by a partner or a GC or a client, "is it safe for me to use the agent on this matter?" The honest answer requires checking four things, in order.
...
|------|---------------------|---------------------------|
| the agent.ai Free | Default opt-in unless changed | No |
| the agent.ai Pro | User-controllable | Marginal — change the setting |
| the agent Team | No (contractual) | Yes |
| the agent Enterprise | No (contractual + DPA) | Yes |
...
- Each lawyer has a *firm-issued* the agent account, not a personal one.
...
**Action.** Get a deployment review on the books for any team using the agent on real matter work. The review should cover: account provisioning, connector inventory, permission grid audit, skill scope audit. Run it quarterly.

**How to verify.** Walk the path of a hypothetical sensitive memo. From the moment a lawyer types it into Hermes session, who could possibly see it before the matter closes? Each "who" is a Layer 3 risk. Reduce the count.
...
This is the layer where general-purpose AI today is genuinely behind. General the agent has the concept of a *project* (a shared workspace with files and instructions). It does not have the concept of a *matter*.
...
**Workarounds in general-purpose the agent.**

- Use Hermes session projects per-matter. One project = one matter. Don't mix.
...
**Action.** If your firm bills time per-matter, your AI usage should also be tracked per-matter. If you can't draw a line from a the agent conversation to a specific matter ID, you have a Layer 4 gap. Decide whether you can live with it given your matter mix and client base. Some firms can. Many cannot.
...
Heppner ruling discussion and the contract/infrastructure framing come from Mark Pike, *the agent for Legal Teams* webinar, April 2026. The deployment and matter layers are HAQQ's editorial expansion based on observed deployments across ~9,800 firms.
