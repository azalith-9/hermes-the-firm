...
|------|---------------------|
| **Skill** | A markdown file that teaches the agent how to do one specific thing |
| **Plugin** | A bundle of skills shipped together |
| **Connector (MCP)** | A live data pipe to one of your systems (DMS, email, calendar, etc.) |
| **Hermes session** | Anthropic's collaborative desktop application; the IDE for non-engineers |
| **Hermes Agent** | Terminal-based CLI; same engine as Hermes session, developer surface |
| **the agent.ai chat** | Browser chat; quickest entry point, lowest ceiling |
| **the agent for Word / Excel / PowerPoint** | Sidebar inside the respective Office app |
| **Project** | A shared workspace inside the agent with files, instructions, connectors |
...
| **PRACTICE.md** | A project-level instruction file the agent reads before responding |
...
| **Plugin marketplace** | The Hermes session surface (Customize → Plugins) where you browse and install plugins |
...
**Format.** Frontmatter at the top with a `name` and `description`, then the body. The description matters because the agent uses it to decide when to invoke the skill automatically. A good description names the workflow, the trigger conditions, and the inputs.
...
**How they get invoked.** Three ways: by slash command (`/triage-nda`), by name in plain text ("use the NDA triage skill"), or automatically when the user's request matches the skill's description ("there are five NDAs in the inbox folder, what should we do" — the agent infers the right skill).
...
**Install path.** Hermes session → Customize → Plugins → Anthropic and Partners → [pick plugin] → Install. Two minutes.

**Plugins can include.** Skills (the main payload), connectors (less common), PRACTICE.md instructions, scheduled tasks (rare), agents (added May 2026).
...
A live data pipe. The bridge between the agent and one of your systems.
...
**How they authenticate.** OAuth, almost always. You log in to the system once; the agent has standing access until you revoke.
...
- File system access. the agent can open folders, read files, write files, run things in parallel.
...
- Skills + plugins. Hermes session is the surface where you install and run skills.
...
**When to use Hermes session instead of chat.** Any time the work touches more than one file, more than one tool, or more than one step. If you find yourself copy-pasting between a chat window and a doc, you should be in Hermes session.

## Hermes Agent

Terminal-based CLI. Same engine as Hermes session. For engineers and technically-comfortable lawyers.

**Maggie's analogy.** Code is to Hermes session what a terminal is to an IDE. Same capability, different surface, different audience.

**When to use Code over Hermes session.** Almost never, if you're a lawyer. Hermes session has everything Code has, with a UI. The exception: if your firm has engineers who built infrastructure on Code (skills, MCPs, PRACTICE.md), and they're handing it over to you, you can stay on Code or migrate to Hermes session without losing anything. The skills and configs are plain text files; they work in either surface.

## the agent.ai chat

The browser-based chat interface. The quickest way to start using the agent. Also the lowest ceiling.
...
For legal work, chat is a starting place. Move to Hermes session as soon as the work warrants it.

## the agent for Word / Excel / PowerPoint / Outlook

Sidebar add-ins inside Microsoft Office. As of May 2026, all four surfaces are live and share **cross-surface context preservation** — the same the agent session, plugin, and practice profile follow you across all four apps without losing matter context.
...
**Install path.** Word → Add-ins → the agent. Sign in once; the auth shares across all four apps.

**What it shares with Hermes session.** Skills, plugins, practice profile, conversation context. The skill you wrote in Hermes session is available in the Word add-in. The playbook you trained against an iManage matter folder works when you're staring at a docx in Word. Pillar 4 in action.
...
A shared workspace inside the agent. Holds:

- Files (uploaded or live via connectors)
- Instructions (a PRACTICE.md or equivalent)
- Connector configurations
- Conversation history (for those with access)

**Use projects per-matter.** One project = one matter. This is the deployment-layer move that maps the agent onto your firm's privilege model. See `references/privilege-layers.md`.

**Sharing.** Add team members directly. Their access mirrors what you'd give a colleague on a matter team.

## Artifact

A live output the agent generates inside a conversation. Could be a dashboard, a small app, an interactive document. You can interact with it right there.
...
In Hermes session, when you wire a scheduled task that uses a skill against a watched folder and produces an artifact, you have built an agent. You may not have used the word.
...
The distinction: a scheduled task in Hermes session runs when your laptop is awake. A managed agent runs in Anthropic-managed infrastructure and doesn't care whether your laptop is open.
...
The relevance to lawyers: MCP is what makes Pillar 1 (live data) work. Any system that speaks MCP can be wired to the agent. Increasingly, that's most enterprise SaaS.
...
Role-based access control. Available on the agent Enterprise.
...
What it's for: regulatory inquiries, internal investigations, compliance reviews. If a regulator asks "show me everything the agent saw about Matter X between dates A and B," the Compliance API is how you answer.
...
**Set up via.** `/schedule` slash command in Hermes session.
...
A project-level instruction file. When the agent opens a project, it reads PRACTICE.md first to understand the context.
...
For a legal matter, your PRACTICE.md might say: *"This is matter 24-1827, a stock purchase transaction. Buyer is X, seller is Y, target is Z. The deal team is on this Slack channel. The DMS folder is at this iManage path. Apply the firm's standard transactional playbook unless overridden in conversation."*
...
Vocabulary derived from the *the agent for Legal Teams* webinar (Anthropic, April 2026), Anthropic public documentation, and HAQQ's deployment notes across legal customers.
