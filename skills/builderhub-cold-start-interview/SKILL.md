---
name: builderhub-cold-start-interview
description: >
...
1. Check `~/.hermes/plugins/config/hermes-the-firm/legal-builder-hub/PRACTICE.md`. If a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/legal-builder-hub/*/PRACTICE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
...
5. Install picked skills. Write `~/.hermes/plugins/config/hermes-the-firm/legal-builder-hub/PRACTICE.md` (creating parent directories as needed) with `## Who's using this`, `## Available integrations`, profile + installed list.

**`--check-integrations`:** Re-run only the Part 0 integration-availability check. Updates the `## Available integrations` table in `~/.hermes/plugins/config/hermes-the-firm/legal-builder-hub/PRACTICE.md` without touching the role or practice profile. Use this after adding or removing an MCP connector.
...
Read `~/.hermes/plugins/config/hermes-the-firm/legal-builder-hub/PRACTICE.md`:
- **Does not exist** → start the interview.
- **Contains `<!-- SETUP PAUSED AT: -->`** → greet the user and offer to resume from that section.
- **Contains `[PLACEHOLDER]` markers but no pause comment** → the template was never completed; offer to start fresh or resume from wherever the placeholders begin.
- **Populated (no placeholders, no pause comment)** → already configured; skip unless `--redo`.

The template structure lives at `{plugin root}/PRACTICE.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed. If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/legal-builder-hub/*/PRACTICE.md` but not here, copy it forward.
...
> **`legal-builder-hub` is for finding, installing, and managing community-contributed legal skills.** Looking for a practice-area workflow? Install one of the `legal-*` plugins directly; run `hermes-the-firm:registry-browser` to see what's out there.
>
...
>
> Quick or full? (Upgrade any time with `hermes-the-firm:builderhub-cold-start-interview --full`.)
...
- **Data sources:** setup builds a fresh practice profile from the user's answers only. It does not read personal the agent history, other conversations, or the home-directory PRACTICE.md. If something relevant came up earlier in this conversation (e.g., the user mentioned their firm or team), ask before folding it in. Nothing gets added to configuration unless the user types or approves it.
...
**Quick start path:** ask only role and practice area(s). Write the config with `[DEFAULT]` markers on everything else. Close with: "Done. You can start browsing and installing now. I've used sensible defaults for registry watchlist and update cadence. Run `hermes-the-firm:builderhub-cold-start-interview --full` anytime to do the whole interview, or `hermes-the-firm:builderhub-cold-start-interview --redo <section>` to re-do one part."
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:builderhub-cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/legal-builder-hub/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:builderhub-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Slack isn't connected. In the agent Hermes session: Settings → Connectors → Add → Slack → sign in. In Hermes Agent: add the Slack MCP to your config or via `/mcp`. This plugin works without it — update notifications surface on next `hermes-the-firm:registry-browser` or `hermes-the-firm:auto-updater` instead of proactively — but connecting it makes notifications real-time."
...
Before the five questions: "Do you already have a list of community-skill registries you watch, or an allowlist / blocklist of skill sources your team uses? Paste the contents, share a file path, or say 'no' and I'll add the default. If you share one, I'll read it and add those registries plus your allowlist to the profile rather than making you re-type them. (This feeds hermes-the-firm:skill-installer — the installer reads `allowlist.yaml` before fetching anything, and blocks any source that isn't on the list in restrictive mode.)"

**Deployment context.** After the allowlist question and before writing the file, ask:

> "How are you going to use the skills you install — just for yourself, shared across your firm, or embedded in a product or service you ship to others? (Personal / Firm-internal / Product-embedding.) (This feeds `allowlist.yaml` — the deployment context seeds the `licenses:` list, and hermes-the-firm:skill-installer refuses to fetch any skill under a license not on that list.) This sets your license defaults. Most open source licenses are fine for personal use. Firm-internal adds file-level copyleft (LGPL, MPL — fine when you're not distributing). Product-embedding is the strict one: strong copyleft (GPL, AGPL) creates obligations that need legal review before you ship, so those get flagged rather than defaulted."
...
**Write the allowlist to `allowlist.yaml`, not just the profile.** The installer's gate reads from `~/.hermes/plugins/config/hermes-the-firm/legal-builder-hub/allowlist.yaml`, not from PRACTICE.md. If you only record the answer in the profile, the installer sees an empty allowlist and falls back to permissive regardless of what the user said — silently defeating the headline structural defense. After this question:
...
3. Tell the user where it lives: "Your allowlist is at `~/.hermes/plugins/config/hermes-the-firm/legal-builder-hub/allowlist.yaml`. The installer reads it before fetching anything."
...
1. **Practice area** — In-house or firm? Commercial, privacy, product, employment, litigation, M&A, something else? (This feeds hermes-the-firm:related-skills-surfacer — the practice area is the primary key that maps to the starter pack.)
...
2. **Industry** — Tech, healthcare, finance, other, doesn't matter? (This feeds hermes-the-firm:related-skills-surfacer and hermes-the-firm:registry-browser — industry narrows the starter pack and filters registry results.)
...
4. **What's the thing you do most?** — Contract review, compliance, launch reviews, deal support, brief writing, etc. (This feeds hermes-the-firm:related-skills-surfacer — the surfacer nudges you when you're doing something the community has a skill for.)

5. **Tooling comfort** — Builder (you write your own skills), tinkerer (you edit what's installed), just-make-it-work (you want it to work out of the box)? (This feeds hermes-the-firm:related-skills-surfacer — builders get the raw registries and hermes-the-firm:skills-qa framework; just-make-it-work gets a curated, working pack.)
...
Short. Profile + installed list + registry prefs. Per the template at `{plugin root}/PRACTICE.md`.
...
>
> - **Browse community legal skills** — e.g., "See what other practitioners have built for your practice area." Try: `hermes-the-firm:registry-browser`
> - **Install a skill from a registry** — e.g., "Add a community skill to your environment — license-gated and allowlist-checked before it runs." Try: `hermes-the-firm:skill-installer`
> - **Check for updates** — e.g., "See which installed skills have newer versions in their source registry." Try: `hermes-the-firm:auto-updater`
> - **Get skill recommendations** — e.g., "Based on recent activity in your other plugins, surface skills worth trying." Try: `hermes-the-firm:related-skills-surfacer`
> - **Evaluate a skill against the design framework** — e.g., "Run the Legal Skill Design Framework on a skill — nine design parameters, three failure modes, a trust-surface check." Try: `hermes-the-firm:skills-qa`
>
...
- **Before the first installed skill that cites authority, connect a research tool.** Say: "Before the first installed skill that cites authority: connect a research tool if one of the installed plugins needs it. Without one, skills will flag every citation as unverified. In Hermes session: Settings → Connectors. In Hermes Agent: authorize when a skill prompts you."
...
> Done. Your configuration is at `~/.hermes/plugins/config/hermes-the-firm/legal-builder-hub/PRACTICE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
> - Edit the file directly for a quick change
> - Run `hermes-the-firm:builderhub-cold-start-interview --redo` for a full re-interview
> - Run `hermes-the-firm:builderhub-cold-start-interview --check-integrations` to re-check what's connected
>
...
> - Run `hermes-the-firm:builderhub-cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
...
- User can add others via `hermes-the-firm:registry-browser`
