...
The output feeds `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/deviation-log.yaml`. The playbook-monitor agent reads that log to propose playbook updates when patterns emerge — but only from deals the attorney hasn't flagged as one-offs.
...
Read `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` in full. Extract:
...
Using the repository location from `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`:

- **If CLM connected:** query for agreements with status = executed/signed in the last 7 days using `an MCP search tool on any connected server` or `an MCP query tool on any connected server`.
...
2. Identify the applicable playbook section(s) from `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`.
...
```
[#] [Deal] — [Clause]
Playbook position: [standard position from `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`]
...
Append a structured entry to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/deviation-log.yaml` for each agreement processed.
...
Logged to: ~/.hermes/plugins/config/hermes-the-firm/commercial-legal/deviation-log.yaml
...
Upstream ran this definition as a scheduled Claude Code agent. Hermes has no self-scheduling agents: run it on demand (or via a `cronjob` that prompts "run the deal-debrief check"), then follow the body below.
