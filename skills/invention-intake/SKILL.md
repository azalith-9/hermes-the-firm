...
1. Read `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`. If it
   contains `[PLACEHOLDER]`, stop and direct to `hermes-the-firm:ip-cold-start-interview`. If the
...
```
hermes-the-firm:invention-intake "a new cache-eviction algorithm that uses a learned model rather than LRU; conceived Q1 this year, not yet disclosed, engineering prototype in internal staging"
```

```
hermes-the-firm:invention-intake
```
...
PRACTICE.md. If `Enabled` is `✗` (the default for in-house users), skip the rest
...
this for? Run `hermes-the-firm:ip-matter-workspace switch <slug>` or say `practice-level`." Load
the active matter's `matter.md` for matter-specific context and overrides.
Write outputs to the matter folder at
`~/.hermes/plugins/config/hermes-the-firm/ip-legal/matters/<matter-slug>/`.
...
`~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`.** If it is
missing or still contains placeholders, stop and run `hermes-the-firm:ip-cold-start-interview`. The
...
**Never do a prior-art search in this skill.** A web_search for "does this
