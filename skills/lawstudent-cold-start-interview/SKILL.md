---
name: lawstudent-cold-start-interview
description: >
...
<!--
HERMES PORT NOTE
Mechanically ported into hermes-the-firm from anthropics/claude-for-legal (plugin 'law-student', Apache-2.0) by tools_dev/port_upstream.py.
Tool names, config paths, and /law-student:{skill} invocations were
...
1. Check `~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md`. If already populated and no `--redo`, confirm before overwriting. If a populated ~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/law-student/*/PRACTICE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
...
5. Write `~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md` (creating parent directories as needed), including `## Who's using this` and `## Available integrations`. Add `LIMITED DATA` flag if fewer than 10 materials were shared.
6. Confirm with the user: "Here's what I captured — anything wrong?"

**`--check-integrations`:** Re-run only the Part 0 integration-availability check. Updates `## Available integrations` in `~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md` without touching the role or the rest of the profile. Use after adding or removing an MCP connector.
...
Read `~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md`:
...
The template structure lives at `{plugin root}/PRACTICE.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed. If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/law-student/*/PRACTICE.md` but not here, copy it forward.
...
> **`law-student` is for law students studying for class or the bar.** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
>
> Quick or full? (Upgrade any time with `hermes-the-firm:lawstudent-cold-start-interview --full`.)
...
- **Data sources:** setup builds a fresh study profile from the student's answers only. It does not read personal the agent history, other conversations, or the home-directory PRACTICE.md. If something relevant came up earlier in this conversation (e.g., a class or a bar date), ask before folding it in. Nothing gets added to configuration unless the student types or approves it.
...
- **Pause and resume.** Tell the student up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:lawstudent-cold-start-interview` again later and I'll pick up where you left off." When the student pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:lawstudent-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the student: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
*(This feeds `hermes-the-firm:study-plan` and `hermes-the-firm:outline-builder` — classes become scheduled study blocks, exam formats drive what `hermes-the-firm:exam-forecast` and `hermes-the-firm:irac-practice` prepare you for, and the bar date schedules `hermes-the-firm:bar-prep-questions` backward from the exam.)*
...
- Bar jurisdiction and target date (if known) (This feeds `hermes-the-firm:bar-prep-questions` — schedules MBE sets and essay practice backward from this date, filtered to your jurisdiction's essay subjects.)
...
*(This feeds `hermes-the-firm:socratic-drill`, `hermes-the-firm:irac-practice`, and `hermes-the-firm:cold-call-prep` — drill-me pushes back without giving you the answer; explain-to-me scaffolds first, then tests. The default can be overridden per session.)*
...
*(This feeds `hermes-the-firm:study-plan` and `hermes-the-firm:bar-prep-questions` — weak areas and avoided subjects get more scheduled time and more drill sessions than strong ones.)*
...
*(This feeds `hermes-the-firm:outline-builder` (your format and depth), `hermes-the-firm:exam-forecast` (professor patterns from past exams), `hermes-the-firm:legal-writing` (your writing voice from graded essays), and `hermes-the-firm:irac-practice` (feedback patterns). Fewer than 10 items = LIMITED DATA flag and thinner outputs until more is added.)*
...
Per the template at `{plugin root}/PRACTICE.md`. Short — it's about one person.

**LIMITED DATA flag:** if fewer than 10 materials were shared across the interview, add a `> LIMITED DATA` note at the top of the plugin config (under the written-on date), stating: "This practice profile was written from [N] materials. Downstream skills will operate but outputs will be thinner — the outline builder doesn't have your format yet, the exam forecast has thin signal on your professors, the IRAC grader won't know your writing patterns. Re-run `hermes-the-firm:lawstudent-cold-start-interview --redo` after gathering more outlines, graded essays, or old exams to sharpen it."
...
>
> - **Brief a case in your format** — e.g., "Opinion in, brief out — in the format you actually use for class." Try: `hermes-the-firm:case-brief`
> - **Grade an IRAC essay** — e.g., "Structure, issue-spotting, rules, analysis, organization — does not rewrite." Try: `hermes-the-firm:irac-practice`
> - **Build or extend a class outline** — e.g., "Your format, your subject, iteratively built as you go." Try: `hermes-the-firm:outline-builder`
> - **Cold-call prep for tomorrow's class** — e.g., "Predict your professor's questions and drill them." Try: `hermes-the-firm:cold-call-prep`
> - **Flashcards by subject with Leitner buckets** — e.g., "Generate, drill, and promote / demote across sessions." Try: `hermes-the-firm:flashcards`
> - **Bar prep questions targeted at weak subjects** — e.g., "MBE or essay, drawn from your weak-subject list." Try: `hermes-the-firm:bar-prep-questions`
>
> **My suggestion for your first one:** Run `hermes-the-firm:case-brief` on the next case you have to read — it'll tell you whether the brief format matches how you actually study. Or tell me what's on your plate and I'll pick.
...
- **Start here:** `hermes-the-firm:study-plan` — builds a study schedule from your classes, exam dates, and weak areas. It'll suggest when to drill, when to outline, and when to do practice exams.
...
> Done. Your configuration is at `~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
> - Edit the file directly for a quick change
> - Run `hermes-the-firm:lawstudent-cold-start-interview --redo` for a full re-interview
> - Run `hermes-the-firm:lawstudent-cold-start-interview --check-integrations` to re-check what's connected
>
...
> - Run `hermes-the-firm:lawstudent-cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
