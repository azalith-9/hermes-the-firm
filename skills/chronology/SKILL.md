...
1. Load `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/matter.md` → theory, pivot fact, key facts.
2. Load `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → Document storage sources, default matter folder pattern.
3. Follow the workflow and reference below.
4. Identify sources in order: user-provided paths this session, default matter folder, declared sources from `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`.
...
8. Write `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/chronology.md` (or format variant per flag).
...
This skill serves two practice settings. Pick a default from the user's `## Role` in the plugin's configuration PRACTICE.md; the user can override per-run with a flag.

- **`--matter` mode (default for in-house litigation counsel).** Matter-history-focused. Reads the matter's case theory and key facts from `matter.md`, pulls from declared document-storage sources (Google Drive, SharePoint, Gmail, iManage, CLM — whatever the `## Landscape` section of PRACTICE.md declares), and treats `history.md` as the running internal log (decisions, holds, reserve memos — intentionally not in the chronology). Output is matter-centric: what happened across the dispute, tagged for advocacy use.
...
Common:
- Plugin configuration PRACTICE.md → case theory context (in-house: `## Landscape` for document sources; firm associate: `## Case theory` and `## Document review` for platform + custodians), `## Outputs` for the work-product header, `## Decision posture` for the privilege-flagging rule.
...
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/matter.md` → case theory, key facts, pivot fact (for significance tagging), key dates.
- Default matter folder pattern from PRACTICE.md → where docs for this slug live.
...
**Conflicts gate — unbypassable (`--matter` mode).** Before building the chronology, check `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:

> "I don't see [matter slug] in the matter log. Run `hermes-the-firm:matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't build a chronology on a matter that hasn't been intaken — the conflicts check is the gate."
...
2. **Default matter folder** — from PRACTICE.md's document-storage pattern, expanded for this slug (e.g., `G:/Legal/Matters/acme-v-us-2026`).
3. **Declared sources** — the `Document storage` table in PRACTICE.md, filtered to ones this matter might touch (e.g., Gmail archive for sender-side communications, SharePoint legal folder).
...
Location: `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/chronology.md`. Complete, tagged, annotated. The reference doc counsel works from.
...
**Unreadable sources:**
[sources declared in PRACTICE.md but not accessible this run — e.g., "Everlaw production — no MCP connector; export needed"]
