...
Reviews an inbound agreement against the playbook in `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`. Identifies the agreement structure from titles, selects the appropriate skill(s), and — if confirm_routing is enabled — checks with the user before proceeding.

## Instructions

1. **Load `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`.** If placeholders present, stop and prompt: "Run `hermes-the-firm:commercial-cold-start-interview` first — I need to learn your playbook before I can review against it."

   Also read `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` → `## Review preferences` → `confirm_routing`. If the field is missing, treat it as `true`.
...
5. **Confirm routing if enabled.**
   ...
   If `confirm_routing` is `true` in `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` (or field is absent):
...
7. **Check for escalations:** If any issue exceeds the reviewer's authority per the `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` matrix, invoke **escalation-flagger** to route and draft the ask.
...
Add to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` → `## Review preferences`:
...
```
hermes-the-firm:review vendor-msa.pdf
```

```
hermes-the-firm:review https://drive.google.com/file/d/ABC123
```

```
hermes-the-firm:review
[paste agreement text]
```

## Output

Full review memo per the skill's format. Routing decision logged at the top. Deviation-by-deviation, specific redline language, named approver. Saved where `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` → House style says work product goes.
