...
1. Load `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md` → framework + calibration. Stop if placeholders.
...
```
hermes-the-firm:launch-review PROJ-1234
```
...
**Matter context.** Check `## Matter workspaces` in the practice-level PRACTICE.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `hermes-the-firm:product-matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.hermes/plugins/config/hermes-the-firm/product-legal/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.
...
Read the PRD, check every category in this team's framework, calibrate against what actually blocks here (per `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md`), and output a review in house format. Goal: a PM reads it and knows exactly what has to happen before they ship.
...
If AI component detected → flag it, then run `hermes-the-firm:aigov-use-case-triage [feature]`
...
For each category in `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md` → Review framework. If the team doesn't have one, use the 8-category default below. The categories are stable framing concepts; within each category, research the regulatory regimes applicable to the product's sector, audience, and jurisdictions before calibrating severity. What blocks in one jurisdiction or sector may be routine in another — `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md` captures the team's calibration.
...
**Calibration:** [per the config PRACTICE.md — this is usually an FYI / usually needs X / usually blocks]
...
For each finding, check against the calibration table in ~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md:
...
Format per `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md` → Launch review process → output format. Prepend the work-product header from `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md` `## Outputs` (it differs by user role — see `## Who's using this`). If no house format is specified:
...
> **Before emitting a "Clear to ship" or "Ship with conditions" call:** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md`. If the Role is Non-lawyer:
>
...
**Output 1 — Privileged launch review memo.** The full analysis assembled in Step 5: work-product header, bottom line, findings by category with risk rationale, action items, escalations, notes for next time, citation check. This is internal legal work product. Keep it in your matter file (Drive, DMS, or wherever `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md` says review docs go). Distribute only to people inside the privilege circle.
...
- **To privacy:** If the launch touches personal data, run `hermes-the-firm:privacy-use-case-triage [feature]`. If triage returns PIA REQUIRED or DPIA MANDATORY, run `hermes-the-firm:pia-generation [feature]`. Don't just note "PIA needed" — trigger it.
- **To AI governance:** If an AI component was detected in Step 2, run `hermes-the-firm:aigov-use-case-triage [feature]`. If triage returns CONDITIONAL, run `hermes-the-firm:aia-generation [feature]`. If a new AI vendor is involved, run `hermes-the-firm:vendor-ai-review [vendor agreement]`.
...
- It doesn't retroactively calibrate. If this launch turns out fine (or badly) in a way that should update the calibration table, a human updates ~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md.
