...
2. Confirm slug exists in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/` and `_log.yaml`.
...
4. Append dated entry to `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/history.md`.
...
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml` — find the row
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/history.md` — append target
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/matter.md` — reference (don't rewrite)
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` — risk calibration (if re-assessing risk)
...
> "I don't see [matter slug] in the matter log. Run `hermes-the-firm:matter-intake` first so the conflicts check runs and the matter workspace exists. I won't append history to an unmanaged matter — the conflicts check is the gate, and there's no `history.md` to append to until the matter is intaken."
...
If the Strategy update is a **settlement acceptance** (the company is accepting a settlement offer, executing a settlement agreement, or authorizing acceptance in principle — not merely logging an offer made or received): Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`. If the Role is Non-lawyer:
...
If materiality moves to `reserved` or `disclosed`, and the matter did not previously carry a reserve or disclosure, flag the event as requiring finance / audit-committee notification per `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` materiality thresholds.
...
### Append to `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/history.md`
...
### Update `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml`
