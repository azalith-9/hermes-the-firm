...
1. Read `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code]/closing-checklist.yaml` and use the modes below.
...
Lives at `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code]/closing-checklist.yaml`. Structure:
...
handoff:
  ...
  severity: "[🔴 / 🟠 / 🟡 / 🟢 — carried from upstream, see severity-floor rule in PRACTICE.md]"
...
```
hermes-the-firm:closing-checklist
...
If the checklist has more than ~10 items, or any time the user asks: offer the dashboard (see PRACTICE.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by status (done / in progress / not started / at risk), a critical-path view grouped by workstream, and a sortable grid with item, owner, due date, and days-to-due.
...
**Before producing a "ready to close / all CPs satisfied" certification or closing memo:** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`. If the Role is **Non-lawyer**:
