...
description: >
  ...
  escalation table. Use on fresh install, when PRACTICE.md still has
...
5. If a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/employment-legal/*/PRACTICE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `~/.hermes/plugins/config/hermes-the-firm/employment-legal/PRACTICE.md`, creating parent directories as needed.
...
Read `~/.hermes/plugins/config/hermes-the-firm/employment-legal/PRACTICE.md`:
- **Does not exist** → start the interview.
- **Contains `<!-- SETUP PAUSED AT: -->`** → greet the user and offer to resume from that section.
- **Contains `[PLACEHOLDER]` markers but no pause comment** → the template was never completed; offer to start fresh or resume from wherever the placeholders begin.
- **Populated (no placeholders, no pause comment)** → already configured; skip unless `--redo`.

The template structure lives at `{plugin root}/PRACTICE.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed. If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/employment-legal/*/PRACTICE.md` but not here, copy it forward.
...
> **`employment-legal` is for people who handle hiring, terminations, investigations, leave, policies, worker classification, and international expansion.** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
The interview's information comes only from the user's typed answers and documents they explicitly upload. Do not read `~/PRACTICE.md`, personal notes, or any ambient context to fill in practice details. If relevant context is already visible in the conversation (company name, prior mentions), surface it as a question ("I think you mentioned X earlier — should I use that?") before using it.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:employment-cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/employment-legal/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:employment-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> - ✗ [Integration] — not found. [Feature] will fall back to [manual alternative]. [How to connect.] If you set this up later, re-run `hermes-the-firm:employment-cold-start-interview --check-integrations`.
>
...
#### Write to the config PRACTICE.md

Write `## Who's using this`, `## Available integrations`, and `## Outputs` sections immediately after the first section of the config-path PRACTICE.md (the plugin config) per the template in `{plugin root}/PRACTICE.md`. These drive work-product header choice and feature-fallback behavior across every skill in this plugin.
...
Per the template structure at `{plugin root}/PRACTICE.md`. Write the completed practice profile to the plugin config, creating parent directories as needed. Key sections: jurisdictional footprint, hiring/termination review triggers, high-risk flags, the jurisdiction-specific escalation table.
...
>
> - **Review an offer letter and restrictive covenants** — e.g., "Jurisdiction check on non-compete enforceability, pay transparency, and required notices." Try: `hermes-the-firm:hiring-review`
> - **Termination review with risk flags** — e.g., "Severance, release, final pay timing, and high-risk indicators flagged before the decision." Try: `hermes-the-firm:termination-review`
> - **Classify a worker engagement** — e.g., "Employee / IC / temp / vendor — with misclassification gap analysis." Try: `hermes-the-firm:worker-classification`
> - **Ask a jurisdiction-aware wage/hour question** — e.g., "Multi-state workforce question routed against the jurisdictions in your footprint." Try: `hermes-the-firm:wage-hour-qa`
> - **Kick off international expansion** — e.g., "New country on the roadmap — plan the employment-law workstream." Try: `hermes-the-firm:expansion-kickoff`
> - **Open an internal investigation** — e.g., "Create the privileged workspace, start the log, route interviews." Try: `hermes-the-firm:investigation-open`
>
...
- If manual leave tracking: "You don't have an HRIS leave module — I'll track leaves in a register file. Use hermes-the-firm:log-leave to add any leaves that are currently open."

**Before your first review**: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Hermes session: Settings → Connectors. In Hermes Agent: authorize when a skill prompts you.
...
> "Done. Your configuration is at `~/.hermes/plugins/config/hermes-the-firm/employment-legal/PRACTICE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
> - Edit the file directly for a quick change
> - Run `hermes-the-firm:employment-cold-start-interview --redo` for a full re-interview
> - Run `hermes-the-firm:employment-cold-start-interview --check-integrations` to re-check what's connected
>
...
> - Run `hermes-the-firm:employment-cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
