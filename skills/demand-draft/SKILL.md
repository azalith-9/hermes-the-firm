...
1. Load `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/demand-letters/[slug]/intake.md`. Refuse if missing or strategic block empty (for material demands).
2. Load `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → demand-letter practice, house style, seed-doc table.
...
5. Template select: seed doc if provided in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`; else soft template for the demand type.
6. Draft in-chat for review. Iterate until user approves.
7. Write `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/demand-letters/[slug]/draft-v[N].docx` using the docx skill.
8. Write `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/demand-letters/[slug]/checklist.md` (post-send checklist).
...
Demand letters are advocacy, and every quoted line from a contract, an email, or a prior communication becomes an assertion the counterparty will test. Canonical statement in the plugin's `PRACTICE.md` shared guardrails; repeated here.
...
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/demand-letters/[slug]/intake.md` — required; refuse to proceed if missing
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → Demand-letter practice (seed-doc paths, insurance-tender timing, materiality threshold for matter creation), house style (privilege markings, outside counsel directive format for tone reference). **Tone, compliance period, marking, and signer come from `## Posture for this matter` — they are matter-level, not practice-level.**
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml` — to check for existing related matters (same counterparty) and offer cross-link
...
Check `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → Demand-letter practice → seed-doc table for the intake's demand type.
...
7. **Privilege markings per house style.** Apply `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` privilege conventions exactly.

## Output

### Primary: `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/demand-letters/[slug]/draft-v[N].docx`
...
### `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/demand-letters/[slug]/checklist.md` — the post-send checklist
...
**Before the letter is sent (the consequential act):** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`. If the Role is Non-lawyer:
...
If material: `hermes-the-firm:matter-intake` with `source: demand-letter` pre-populated from this intake.
```
...
- **Default yes if ANY of:**
  ...
  - Desired outcome $$ ≥ `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` medium-severity band
...
> Create a tracked matter in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml`? (default: [yes/no])
...
If user declines: update intake `status: drafted` (later `sent` when user confirms). The record stays in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/demand-letters/` only.
