...
description: >
  ...
  `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` from
...
<!--
HERMES PORT NOTE
Mechanically ported into hermes-the-firm from anthropics/claude-for-legal (plugin 'ai-governance-legal', Apache-2.0) by tools_dev/port_upstream.py.
Tool names, config paths, and /ai-governance-legal:{skill} invocations were
...
1. Check `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` — if populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/ai-governance-legal/*/PRACTICE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` (create parent directories as needed). Show summary. Offer first task.

## Flags

- `--redo` — re-run the full interview and overwrite `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` without re-running the full interview. Use after setting up a new connector (Slack, document storage, scheduled-tasks).
...
```
hermes-the-firm:aigov-cold-start-interview
hermes-the-firm:aigov-cold-start-interview --check-integrations
```
...
Read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`:
...
If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/ai-governance-legal/*/PRACTICE.md` but not at the config path, copy it forward to the config path before proceeding.
...
> **`ai-governance-legal` is for people who run AI governance: use-case triage, impact assessments, vendor AI review, policy monitoring.** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
**Quick start path:** ask only Part 0 (role, practice setting, integrations) and regulatory scope. Write the config with `[DEFAULT]` markers on everything else. Close with: "Done. You can start using the commands now. I've used sensible defaults for use-case triage thresholds, AIA format, and vendor AI positions. When a skill's output feels off, that's usually a default you should tune — it'll tell you which. Run `hermes-the-firm:aigov-cold-start-interview --full` anytime to do the whole interview, or `hermes-the-firm:aigov-cold-start-interview --redo <section>` to re-do one part."
...
> "This plugin maintains: your practice profile (governance tiers, red lines, policy commitments), a use-case registry, impact assessments, and vendor AI reviews — all in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/`. It learns how you actually work — your practice, your risk calibration, your house conventions — and writes that into a plain-text file the plugin reads from every time. Everything you answer can be changed later."
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:aigov-cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:aigov-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Google Drive isn't connected. In the agent Hermes session: Settings → Connectors → Add → Google Drive → sign in. In Hermes Agent: add the Drive MCP to your config or via `/mcp`. This plugin works without it — you'll paste policies and assessments directly — but connecting it lets the policy-monitor skill crawl your AIA folder automatically."
...
You don't need all of these. Core features work with file access alone. If you set something up later, re-run `hermes-the-firm:aigov-cold-start-interview --check-integrations`.
...
> add the rest later with `hermes-the-firm:ai-inventory add`. Or skip
...
with `hermes-the-firm:ai-inventory add` later?" If they accept, run the
...
`~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/ai-systems.yaml`.
...
- **Informally adopted tools:** Employees using ChatGPT, Gemini, the agent, Perplexity, or other consumer AI without central approval. Check with IT for SaaS spend, browser extension usage, and DLP alerts.
...
*Re-run: `hermes-the-firm:aigov-cold-start-interview --redo`*
```
...
>
> - **Review vendor AI terms** — e.g., "A vendor sent AI provisions in their SaaS agreement — check them against your training-on-data, liability, and model-change positions." Try: `hermes-the-firm:vendor-ai-review`
> - **Triage a proposed AI use case** — e.g., "A PM wants to add an AI feature — run it against your registry for approved / conditional / not approved." Try: `hermes-the-firm:aigov-use-case-triage`
> - **Run an AI impact assessment** — e.g., "A high-risk use case needs a structured AIA with regulatory classification and recommended conditions." Try: `hermes-the-firm:aia-generation`
> - **Diff a new AI regulation against your posture** — e.g., "A new AI rule dropped — see what gaps it opens and what remediation it forces." Try: `hermes-the-firm:aigov-reg-gap-analysis`
> - **Sweep for policy drift** — e.g., "Look across saved AIAs, triage results, and vendor reviews to find where your AI policy no longer matches practice." Try: `hermes-the-firm:aigov-policy-monitor`
>
...
5. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your configuration is at `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
   > - Edit the file directly for a quick change
   > - Run `hermes-the-firm:aigov-cold-start-interview --redo` for a full re-interview
   > - Run `hermes-the-firm:aigov-cold-start-interview --check-integrations` to re-check what's connected
   >
...
6. **Before your first triage**: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Hermes session: Settings → Connectors. In Hermes Agent: authorize when a skill prompts you.
