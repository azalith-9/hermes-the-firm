...
1. Load `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → role, work-product header, decision posture, document storage.
...
Check `## Matter workspaces` in the practice-level PRACTICE.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `hermes-the-firm:litigation-matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` — especially the case theory, the pleading / complaint (for the elements actually alleged), the jurisdiction, any Markman order or stipulated constructions (patent mode), and the phase of the case. Write outputs to the matter folder at `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/<matter-slug>/claim-charts/`. Never read another matter's files unless `Cross-matter context` is `on`.

---

## Load context

- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → role, work-product header, decision posture, document storage, case-theory scaffolding
...
If `PRACTICE.md` has `[PLACEHOLDER]` markers, surface this bounce:
...
>
> **Two choices:**
> - Run `hermes-the-firm:litigation-cold-start-interview` (2 minutes) to configure your profile, then I'll run this tailored to YOUR practice.
...
> "That was a generic run against default assumptions. Run `hermes-the-firm:litigation-cold-start-interview` to get output calibrated to YOUR practice — your risk calibration, your landscape, your house style. 2 minutes."

**Conflicts gate — unbypassable.** Before building a claim chart, check `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:

> "I don't see [matter slug] in the matter log. Run `hermes-the-firm:matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't build a claim chart on a matter that hasn't been intaken — the conflicts check is the gate."
...
Prepend the work-product header from `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` `## Outputs`.
...
If matter workspaces enabled and a matter is active: `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/<matter-slug>/claim-charts/`. Otherwise: `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/claim-charts/`. Surface the path. Append a one-line entry to the matter's `history.md`.
