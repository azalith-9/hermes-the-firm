...
1. Load `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → case theory, house style.
...
Two rules that govern every citation and every quotation in advocacy drafting. The canonical statement lives in the plugin's `PRACTICE.md` shared guardrails; repeated here because this skill is the most common place the rule gets tested.
...
`~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → case theory, house style (citation format, structure, tone, length norms).

**Conflicts gate — unbypassable.** Before drafting, check `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml` for the matter slug this skill is being invoked on. If the matter is not in `_log.yaml`, refuse and route:

> "I don't see [matter slug] in the matter log. Run `hermes-the-firm:matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't draft substantive work product on a matter that hasn't been intaken — the conflicts check is the gate."
...
Per `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`:
...
**Before the brief is filed (the consequential act — this skill drafts, but the gate runs at the filing step regardless of who triggers it):** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`. If the Role is Non-lawyer:
