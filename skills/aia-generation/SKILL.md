...
description: >
  ...
  assessment in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`.
...
1. Read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`. Confirm impact assessment house style is populated.
...
6. Policy diff against `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` AI policy commitments.
7. Output: assessment doc + conditions list + handoff flags (privacy PIA, vendor review if needed).

```
hermes-the-firm:aia-generation "AI résumé screening for HR"
```
...
**Matter context.** Check `## Matter workspaces` in the practice-level PRACTICE.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `hermes-the-firm:aigov-matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.
...
- Is the use case in the elevated or high governance tier per `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`?
...
Before intake, determine which track to run. The tier definitions and the fast-track criteria come from `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` (`## Use case registry` and `## Governance tiers`), not from any hardcoded regime-specific framework.
...
**Fast track vs. full assessment:** `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` defines what qualifies for abbreviated treatment. If `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` doesn't define fast-track criteria, default to full assessment and ask the user what criteria they want captured for next time.
...
For each regime in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` → `## Regulatory footprint` that applies to this system — **plus any regime surfaced by the re-derivation above** — research the currently operative risk classification framework and determine where the system lands.
...
**Use the seed structure from `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`.** If none was captured, use this default:
...
|---|---|---|
| [commitment from `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` AI policy section] | 🟢 / 🟡 / 🟠 / 🔴 | |
...
**Privacy review required?** [Yes — run `hermes-the-firm:pia-generation`, if the plugin is installed /
No]
...
**Before certifying the AIA (the Sign-off step, marking Status: APPROVED):** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`. If the Role is Non-lawyer:
...
Every assessment should cross-check against the AI policy commitments in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`.
Common drift:
...
- **To privacy:** If personal data is involved, flag: "Run `hermes-the-firm:pia-generation [system name]` in parallel, if the plugin is installed — the AIA doesn't substitute for a PIA."
- **To vendor-ai-review:** If a new vendor is involved, flag: "If there's no AI addendum reviewed for [vendor], run `hermes-the-firm:vendor-ai-review` before production."
