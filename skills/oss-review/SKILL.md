...
Runs an open source license compliance check against the practice profile in `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`. Classifies dependencies by license family, maps obligations to the deployment model, flags license-unknown and non-OSI-posing-as-OSS packages, and recommends actions — comply, replace, remove, seek legal review, seek commercial license.

## Instructions

1. **Load `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`.** If placeholders present, stop and prompt: "Run `hermes-the-firm:ip-cold-start-interview` first — I need to learn your practice profile (and OSS policy, if any) before I can review." If the practice profile points at an uploaded OSS policy, read that too — it is the source of truth for accepted / review / banned licenses on this team.
...
```
hermes-the-firm:oss-review ~/code/my-project/package.json
hermes-the-firm:oss-review ~/code/my-project/requirements.txt
hermes-the-firm:oss-review redis
hermes-the-firm:oss-review ~/code/my-project  # repo root — scan all manifests
```
...
**Before scanning dependencies, read `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`.** If it is missing or still contains placeholders, stop and run `hermes-the-firm:ip-cold-start-interview`. The practice profile tells you:
...
Prepend the work-product header from `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` → `## Outputs` (differs by user role — see `## Who's using this`).
