...
1. Read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` → outputs folder path, AI policy document, last sweep date.
...
6. Flag any use cases in outputs not yet added to the `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` registry.
7. Present results to the human. Only after acknowledgment, update `Last policy sweep` and `gaps_found` in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`.

**Direct query mode** (with description argument):
1. Read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` → current policy commitments, use case registry, actual policy document.
...
**Recurring runs:**
Set up a recurring reminder in your own scheduler to run `hermes-the-firm:aigov-policy-monitor` weekly. Scheduled execution requires a scheduled-tasks integration, which is not bundled with this plugin.

```
hermes-the-firm:aigov-policy-monitor
hermes-the-firm:aigov-policy-monitor "We want to use AI to automatically flag expense reports for review"
```
...
> enable the crawl sweep, run `hermes-the-firm:aigov-cold-start-interview` and provide the outputs
> folder path."

Read the actual AI or acceptable use policy document from the path in `## Outputs`
→ **AI policy document**. The commitments in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` are a summary; the actual
...
**Do not update `Last policy sweep` or `gaps_found` automatically.** After the sweep results are presented, wait for the human to acknowledge them ("sweep acknowledged," "results reviewed," or equivalent). Only then update `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`:
...
- Extract: vendor added, data use terms agreed to, any AI-specific provisions
  accepted that differ from standard positions in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`
...
[Any use cases approved since the last sweep that aren't yet in the `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`
...
[If this use case isn't in the registry: "Add to `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` → Use case registry:"]
```
...
The weekly sweep is designed to run on a recurring cadence. Set up a recurring reminder in your own scheduler to run `hermes-the-firm:aigov-policy-monitor` weekly. Scheduled execution requires a scheduled-tasks integration, which is not bundled with this plugin.
