...
description: >
  ...
  every action. Loaded by the hermes-the-firm:uninstall and
  hermes-the-firm:disable skills.
...
- The skill's files must resolve to a path outside the built-in plugin
  directories that ship with hermes-the-firm.
...
The 12 core plugins that ship with hermes-the-firm are off-limits from this
command. The canonical list lives in the hub's PRACTICE.md under "Built-in
...
wrote to the user's `~/.hermes/plugins/config/...` — surface this to the user
...
### Step 5: Log and update PRACTICE.md
...
- Delete user configuration by default. Configs in
  `~/.hermes/plugins/config/hermes-the-firm/<plugin>/` are preserved unless
