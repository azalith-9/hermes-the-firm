...
1. Load `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` → supervision style, plain-language standards, jurisdiction.
...
```
hermes-the-firm:status client
```

```
hermes-the-firm:status internal
```

```
hermes-the-firm:status court
```
...
`~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` → supervision style, plain-language standards (for client-facing), jurisdiction.
...
**Reader:** The client. Probably stressed. Possibly unfamiliar with legal process. Reading level per `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` plain-language standards (default 6th grade).
...
**Before sending:** sending a client status update is a consequential action. The gate is the supervision workflow in `## Supervision style` in `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md`, reinforced by the Part 0 role check confirming a licensed supervising attorney owns the setup. Confirm the draft has been reviewed per the supervision protocol (queue / flag / lighter-touch) and all internal review labels (`[AI-ASSISTED DRAFT]`, `[VERIFY]`, etc.) have been removed from the client-facing copy.
...
Per `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md`:
