...
1. Load `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` + `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code]/deal-context.md`.
...
**Matter context.** Check `## Matter workspaces` in the practice-level PRACTICE.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `hermes-the-firm:corporate-matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.

---

## Purpose

The VDR has 2,000 documents. Somewhere in there are the 30 that matter for the deal. This skill reads documents against the diligence categories and materiality thresholds from `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`, extracts issues, and writes them in house memo format.

## Load context

- `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` → Diligence structure (categories, materiality thresholds)
- `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` → Issues memo format (how findings are stated)
- `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code]/deal-context.md` → deal-specific thresholds, VDR location
...
Per `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` / deal-context thresholds. Don't review everything if the threshold says contracts >$X.
...
Per the finding template in `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`. If the seed memo used this:
...
- **To ai-tool-handoff:** If Luminance/Kira is in use per `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`, hand bulk contract review there. This skill handles the nuanced documents (side letters, amendments, anything the AI tool struggles with).
...
If the extraction surfaced more than ~10 issues, or any time the user asks: offer the dashboard (see PRACTICE.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by severity (🔴 / 🟠 / 🟡 / 🟢), counts by house category, and a sortable grid of issues with materiality, category, and VDR source.
...
- It doesn't replace bulk AI review. For high-volume clause extraction, hand off to Luminance/Kira per `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`. This skill is for the judgment layer.
