...
description: >
  ...
  Michigan reference corpora (court-rules, mi-statutes-debt) live in this
  skill's `references/` subdirectory, and federal/UCC text is served by the
  firm's open-us-law corpus via the `us-statute-lookup` skill.
version: 0.1.0
---
...
`us-mi-legal-corpus` plugin. Thin-adapted: the federal/UCC corpora it
...
`mi-law-references` skill, references/federal-layer.md).
-->
...
practice. Other skills in the `us-mi-legal-corpus` plugin point here for
...
├── mi-statutes-debt/       # Curated Michigan Compiled Laws chapters for civil practice
└── federal-layer.md        # Routing note: FDCPA / FCRA / Reg-F / UCC / Title 11 text
                            #   is served by the open-us-law corpus via us-statute-lookup
```

The Michigan-specific corpora (`court-rules/`, `mi-statutes-debt/`) and the
seven curated `*.md` index files live physically under this skill. In the
upstream `claude-legal` plugin three more corpora (`federal-debt-laws/`,
`federal-bankruptcy/`, `ucc-model/`) were symlinked in from the shared
`us-federal-debt-corpus` plugin; in **hermes-the-firm** that federal/UCC text
is not copied but served by the open-us-law corpus — see
`references/federal-layer.md` for exactly how each federal citation resolves.
...
- `references/mi-statutes-debt/` — curated Michigan Compiled Laws chapters for civil practice
- `references/federal-layer.md` — routing note for FDCPA / FCRA / Reg-F / UCC / Title 11 text
  (served by the open-us-law corpus via `us-statute-lookup`)
