...
1. Load `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` → clinic profile, semester dates, supervision style.
2. Load `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/deadlines.yaml` and `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/client-comms/[case-id]/log.md` per case.
...
5. Generate per-case handoff memo → `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/handoffs/[semester]/[case_id].md`.
6. Generate cohort summary → `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/handoffs/[semester]/_summary.md`.
...
- `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` → clinic profile, semester, practice areas, supervision style
- `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/deadlines.yaml` → all active deadlines, grouped by case
- `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/client-comms/[case-id]/log.md` (per case) → communications history
...
- Pull all active cases (from intake records + `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/deadlines.yaml` case_ids + client-comms folders)
...
*Pulled from `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/deadlines.yaml`. Incoming student's first job is to confirm these are accurate and owned.*
...
*From `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/client-comms/[case-id]/log.md`. Three-line summary here; incoming student reads the full log.*
...
After all per-case memos, produce `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/handoffs/[semester]/_summary.md`:
...
Closing a case or transitioning it to a new student is a consequential action. The gate is the supervision workflow in `## Supervision style` in `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md`, reinforced by the Part 0 role check confirming a licensed supervising attorney owns the setup. Case-closing memos always get professor sign-off before the case is marked closed in the handoff document, regardless of supervision-style choice.

Per `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` supervision style:
...
Once reviewed, handoff memos live at `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/handoffs/[semester]/[case_id].md`. The incoming student reads them during their `/ramp` run at the start of next semester — `/ramp` should surface the memos for cases the new student is assigned.

## Integration

- **`/ramp`:** at the start of next semester, reads `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/handoffs/[most-recent-semester]/` and surfaces per-case memos for the cases each new student is taking on.
...
- **Close cases.** Handoff is for cases transitioning to the next cohort. Cases closing at semester end should get a final internal status memo (`hermes-the-firm:status internal`) for the file and be marked closed in the handoff document; the status skill supports `client | internal | court` audiences.
