...
1. Load `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → DSAR process (systems list, verification method, SLA).
...
Read `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → `## DSAR process`. That section has:
...
Per the method in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`. Common approaches:
...
Walk the systems list from `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`. For each system:
...
**Before proceeding to send either letter to the data subject:** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`. If the Role is Non-lawyer:
...
> **Note:** Both DSAR letters are externally-facing deliverables sent to the data subject. Do **not** include the work-product header from `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` `## Outputs` on either letter. Internal notes, logs, and exemption analyses that accompany the letters are attorney work product — keep those separate and prepend the work-product header per `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` `## Outputs` (which differs by user role — see `## Who's using this`).
...
Per `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → Escalation table, escalate when:
...
If `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → `## DSAR process` records an internal SLA that is tighter than the legal deadline, use the internal SLA and note the legal backstop.
