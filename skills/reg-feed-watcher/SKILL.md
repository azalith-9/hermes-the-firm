...
1. Load `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md` → watchlist, materiality threshold, feed config.
...
`~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md` → watchlist, materiality threshold, feed configuration, digest output path (if set).
...
Before running the pull, compare the watchlist + feed configuration in PRACTICE.md against `references/source-catalog.md`:
...
> **Coverage gap noticed:** Your watchlist includes [category], but only [N] feeds are configured. The source catalog lists [X] options in this category (e.g., [top 2-3 names]). Want me to suggest additions? Run `hermes-the-firm:regulatory-cold-start-interview --redo` to update, or edit `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md` directly.

Don't nag the same gap repeatedly — if the user has explicitly said "skip state AGs for now," respect that. Note state in PRACTICE.md so it sticks.
...
- **Direct regulator RSS** — fetch and parse any RSS URLs in ~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md feed
...
Each item gets a materiality tier per `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`:
...
| ANPR (Advance Notice of Proposed Rulemaking) | Review-worthy for **strategy**, not compliance — no imposed requirements yet, but signals direction and carries a real comment deadline. Log the comment deadline. Route to `hermes-the-firm:policy-diff` only as a pre-positioning analysis, not as a gap-closure diff. |
...
- **Do** log the comment deadline to `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/comment-tracker.yaml` with `item_type: ANPR` or `item_type: RFI` so the downstream tracker can distinguish these from compliance gaps.
- **Do** include in the digest entry a line that says explicitly: "Pre-rule. Comment deadline [date]. Route to `hermes-the-firm:policy-diff` only as a pre-positioning analysis (no compliance gap yet)." This primes the policy-diff skill to use its compressed pre-positioning branch rather than a full gap-closure diff.
...
- If comment tracking is enabled in ~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md: append to `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/comment-tracker.yaml`
  with status "undecided" and the default comment decision owner from ~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md
...
The digest goes into the chat by default. **Also write it to a shareable file** whenever the output contains one or more items above FYI, unless the user's PRACTICE.md explicitly sets `Digest output → chat only`.

**File output behavior:**

1. Look for `Digest output path` in `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`. If set, write there. Default if unset: `~/regulatory-legal-digests/reg-digest-YYYY-MM-DD.md`.
...
**Comment tracker:** [N] NPRMs with open comment decisions — run hermes-the-firm:comments to review
...
This skill reads the watchlist, materiality threshold, and feed configuration from `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`. When a required value is still `[PLACEHOLDER]` or empty, say so in the output — specifically, not generically:

- **Watchlist empty:** stop and say "The watchlist in your configuration is empty. I can't pull feeds without knowing which regulators to watch. Run `hermes-the-firm:regulatory-cold-start-interview --redo` or edit `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md` and add at least one regulator."
- **Materiality threshold empty:** fall back to the default tiers and append: "This output used the default materiality tiers because your configuration doesn't have custom thresholds set. Tune them with `hermes-the-firm:regulatory-cold-start-interview --redo` or by editing `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`."
- **Feed configuration empty:** run Federal Register API only and append: "This output used only the free Federal Register API because your configuration doesn't list direct RSS or paid feeds. Add feeds with `hermes-the-firm:regulatory-cold-start-interview --redo` or by editing `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`."
...
- Change the materiality threshold. If the filter is wrong, edit ~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md.
