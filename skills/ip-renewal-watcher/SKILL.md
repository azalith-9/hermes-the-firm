...
description: >
  ...
  channel named in `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`
...
1. Read `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` to
...
- Modify the register. It reads and reports; additions come from
  `hermes-the-firm:portfolio --add`, updates come from `--update`, sync comes
...
Upstream ran this definition as a scheduled Claude Code agent. Hermes has no self-scheduling agents: run it on demand (or via a `cronjob` that prompts "run the ip-renewal-watcher check"), then follow the body below.
