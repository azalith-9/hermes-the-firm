...
1. Read `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → outputs folder path, policy document, last sweep date.
...
6. Update Last policy sweep date in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`.

**Direct query mode** (with description argument):
1. Read `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → current policy commitments + actual policy document.
...
**Schedule:** Set up a recurring reminder in your own scheduler (calendar, task manager, or CI) to run `hermes-the-firm:privacy-policy-monitor` weekly. Scheduled execution requires a scheduled-tasks integration, which is not bundled with this plugin.

```
hermes-the-firm:privacy-policy-monitor
hermes-the-firm:privacy-policy-monitor "We want to start using behavioral data to personalize onboarding emails"
```
...
> the crawl sweep, run `hermes-the-firm:privacy-cold-start-interview` and provide the outputs
> folder path."

Read the actual privacy policy document from the path in `## Outputs` → **Privacy
policy document**. The commitments in the config PRACTICE.md are a summary; the actual document
...
Update **Last policy sweep** in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` to today's date after completing the sweep.
...
- Match the voice and style of the existing policy (read the actual document, not
  just the config PRACTICE.md summary, before drafting)
...
to run `hermes-the-firm:privacy-policy-monitor` weekly. Scheduled execution requires a
...
`~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`, so the next sweep only looks at new files.
...
If the sweep surfaced more than ~10 drift findings, or any time the user asks: offer the dashboard (see PRACTICE.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by surface (policy clause / PIA / DPA / triage), counts by severity, and a sortable grid of findings with source artifact and recommended remediation.
