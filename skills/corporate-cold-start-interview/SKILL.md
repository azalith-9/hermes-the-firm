...
description: >
  ...
  plugin config. Use on fresh install, when PRACTICE.md still has [PLACEHOLDER]
...
<!--
HERMES PORT NOTE
Mechanically ported into hermes-the-firm from anthropics/claude-for-legal (plugin 'corporate-legal', Apache-2.0) for Hermes Agent.
...
1. Check `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`. If `--new-deal`, skip to per-deal setup. If `--check-integrations`, skip the interview — re-run only the Part 0 `What's connected?` check and rewrite the `## Available integrations` table in `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`. When probing: only report ✓ if an MCP tool call actually succeeded. Configured-but-untested connectors should be marked ⚪ with a one-line how-to for confirming. Never report ✓ based on `hermes mcp` configuration declarations alone — that misleads users into thinking something is wired up when it isn't.
...
5. Migration: if a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/corporate-legal/*/PRACTICE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` (create parent directories as needed). For `--new-deal`, write `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code]/deal-context.md`.
...
Read `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`:
...
If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/corporate-legal/*/PRACTICE.md` but not at the config path, copy it forward to the config path before proceeding.
...
> **`corporate-legal` is for people who support M&A deals, board and corporate governance, public company compliance, and entity management.** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
>
> Quick or full? (Upgrade any time with `hermes-the-firm:corporate-cold-start-interview --full`.)
...
<!-- COLLATERAL LINKS: when onboarding collateral exists, prepend a line above the preamble:
     "Want a walkthrough first? [Watch the 3-minute intro](URL) or [read the getting-started guide](URL), then come back and run hermes-the-firm:corporate-cold-start-interview." -->
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:corporate-cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:corporate-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> - ✗ [Integration] — not found. [Feature] will fall back to [manual alternative]. [How to connect.] If you set this up later, re-run `hermes-the-firm:corporate-cold-start-interview --check-integrations`.
>
...
>
> Tell me the numbers that apply. You can always add a module later with `hermes-the-firm:corporate-cold-start-interview --module [name]`.
...
>
> If you don't have shareable minutes right now, you can add them later with `hermes-the-firm:corporate-cold-start-interview --module board`. The board-minutes skill will prompt you for them if they're missing.
...
>
> - **Extract diligence issues from the VDR** — e.g., "Point at a VDR folder and get findings categorized per your house materiality thresholds." Try: `hermes-the-firm:diligence-issue-extraction`
> - **Build the material contracts schedule** — e.g., "From diligence findings, build the disclosure schedule in the purchase agreement's format." Try: `hermes-the-firm:material-contract-schedule`
> - **Draft a board or committee written consent** — e.g., "Precedent search from your consents repository, then drafted in house format." Try: `hermes-the-firm:written-consent`
> - **Entity compliance tracker** — e.g., "See what filings are due in the next 30 / 60 / 90 days across your subsidiaries." Try: `hermes-the-firm:entity-compliance`
> - **Closing checklist status** — e.g., "What's left to close — conditions, documents, consents, filings — with critical path." Try: `hermes-the-firm:closing-checklist`
> - **Post-closing integration** — e.g., "Phased workplan, consent tracking, contract assignment at scale for a just-closed deal." Try: `hermes-the-firm:integration-management`
>
> **My suggestion for your first one:** If you have an active deal, run `hermes-the-firm:closing-checklist` — it shows immediately where the plugin fits in your workflow. Or tell me what's on your plate and I'll pick.
...
> "Before your first diligence extraction or consent: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Hermes session: Settings → Connectors. In Hermes Agent: authorize when a skill prompts you."
...
> - [If M&A active: "When a deal comes in, run `hermes-the-firm:corporate-cold-start-interview --new-deal` to set up deal-specific context on top of the house approach. M&A skills available now: diligence extraction, deal team summaries, material contracts schedule, closing checklist, and post-closing integration."]
> - [If Board & Secretary active: "Board skills available now: `hermes-the-firm:written-consent` for written consents, and the board-minutes skill for drafting minutes in your house format."]
> - [If Entity Management active: "Entity skill available now: `hermes-the-firm:entity-compliance` initializes a compliance tracker from your entity list and surfaces what's due."]
...
> "Your practice profile is at `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
>
> - Edit the file directly for a quick change (a new threshold, a jurisdiction added, a committee renamed)
> - Run `hermes-the-firm:corporate-cold-start-interview --redo` for a full re-interview
> - Run `hermes-the-firm:corporate-cold-start-interview --module [m&a | board | public | entities]` to add or refresh one module
> - Run `hermes-the-firm:corporate-cold-start-interview --check-integrations` to re-check what's connected
>
...
> - Run `hermes-the-firm:corporate-cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
...
Write to `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code-name]/deal-context.md`. Skills read both the plugin config (house) and `deal-context.md` (this deal), with deal-context.md taking precedence on conflicts.
