...
license: MIT
metadata: " id: draft.incorporation-package-LB category: draft practice_area: corporate jurisdictions: [LB] priority: P1 intent: [Lebanon incorporation, company formation Lebanon, SAL, SARL, Commercial Registry, MOET] related: [draft-incorporation-package-difc, draft-employment-contract-lb, kb-corporate-lb, draft-shareholders-agreement] source: Louis — HAQQ Legal AI (github.com/sboghossian/mini-hermes-the-firm) version: '1.0'"
---
...
1. **LBP capital without USD peg** — specifying only LBP capital means the company's registered capital has collapsed in real value; specify USD equivalent or dual-currency.
2. **Wrong structure** — SAL for a two-person startup (which needs only a SARL); unnecessary complexity and cost.
3. **Missing Articles publication** — SALs must publish in the Official Gazette; omission may affect the validity of the SAL.
4. **Foreign-ownership restriction not checked** — Decree 11614/1969 applies to certain sectors; foreign investors should confirm their sector is unrestricted before filing.
5. **NSSF enrollment delayed** — NSSF must be registered before the first employee starts; late enrollment triggers back-contributions.
6. **No statutory auditor for SAL** — SALs are legally required to appoint a commissaire aux comptes; operating without one is non-compliant.
