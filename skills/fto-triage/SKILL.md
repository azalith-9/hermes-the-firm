...
```
hermes-the-firm:fto-triage "an on-device speech recognition model for consumer wearables, US launch first"
```

```
hermes-the-firm:fto-triage
```
...
Before running triage, read `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`. Pull:
...
If `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` contains `[PLACEHOLDER]` or `[Your Company Name]`, surface this bounce:
...
Prepend the work-product header from `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` `## Outputs`. Mark the document as privileged if the role is lawyer; see the non-lawyer gate below if not.
...
`~/.hermes/plugins/config/hermes-the-firm/ip-legal/matters/<matter-slug>/outputs/fto-triage-<subject-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.hermes/plugins/config/hermes-the-firm/ip-legal/outputs/fto-triage-<subject-slug>-YYYY-MM-DD.md`
...
- **Handle trade-secret or trademark analysis** — use `hermes-the-firm:infringement-triage`
  with the right mode.
