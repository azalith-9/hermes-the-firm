...
description: >
  Run the cold-start interview — learns your privacy practice and writes PRACTICE.md
  from your policy, DPA template, and a reference PIA. Use on first run, when
  PRACTICE.md is missing or has placeholders, or when the user says "set up the
...
<!--
HERMES PORT NOTE
Mechanically ported into hermes-the-firm from anthropics/claude-for-legal (plugin 'privacy-legal', Apache-2.0) by tools_dev/port_upstream.py.
Tool names, config paths, and /privacy-legal:{skill} invocations were
...
1. Check `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` — if populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/privacy-legal/*/PRACTICE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` (create parent directories as needed). Show summary. Offer first task.

## `--check-integrations`

Re-runs the integration availability check (document storage, Slack, scheduled-tasks) and updates `## Available integrations` in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
```
hermes-the-firm:privacy-cold-start-interview
```

```
hermes-the-firm:privacy-cold-start-interview --check-integrations
```
...
Read `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`:
...
If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/privacy-legal/*/PRACTICE.md` but not at the config path, copy it forward.
...
> **`privacy-legal` is for people who run the privacy program: PIAs, DPA reviews, DSAR responses, regulatory gap analysis.** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
>
> Then: "Setup builds a fresh professional profile from your answers. It does not read your personal the agent history, other conversations, or your home-directory PRACTICE.md. If I notice relevant information in our conversation context — e.g., you mentioned your company earlier — I'll ask before using it. Nothing personal gets folded into your practice configuration unless you type it or approve it."
>
...
Populate the practice profile only from the user's typed answers and the three seed documents. Do not read `~/PRACTICE.md` or pull practice facts from ambient context. If something relevant is already visible in the conversation, ask before using it.

**Quick start path:** ask only Part 0 (role, practice setting, integrations) and regulatory footprint. Write the config with `[DEFAULT]` markers on everything else. Close with: "Done. You can start using the commands now. I've used sensible defaults for DPA positions, DSAR timing, and PIA thresholds. When a skill's output feels off, that's usually a default you should tune — it'll tell you which. Run `hermes-the-firm:privacy-cold-start-interview --full` anytime to do the whole interview, or `hermes-the-firm:privacy-cold-start-interview --redo <section>` to re-do one part."
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:privacy-cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:privacy-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> - ✗ [Integration] — not found. [Feature] will fall back to [manual alternative]. [How to connect.] If you set this up later, re-run `hermes-the-firm:privacy-cold-start-interview --check-integrations`.
>
...
*Re-check: `hermes-the-firm:privacy-cold-start-interview --check-integrations`*
...
*Re-run: `hermes-the-firm:privacy-cold-start-interview --redo`*
```
...
>
> - **Review a DPA against your playbook** — e.g., "Auto-detects processor vs. controller; flags deviations from your positions." Try: `hermes-the-firm:dpa-review`
> - **Triage a processing activity** — e.g., "PIA, mandatory GDPR DPIA, or proceed — with privacy-policy conflict surfaces." Try: `hermes-the-firm:privacy-use-case-triage`
> - **Generate a PIA in house format** — e.g., "Structured intake, risk analysis, regulatory classification, recommendation." Try: `hermes-the-firm:pia-generation`
> - **Walk through a DSAR** — e.g., "Verify, locate, assess exemptions, draft the response letter." Try: `hermes-the-firm:dsar-response`
> - **Diff a new regulation against your policy** — e.g., "Outputs the gap list and a remediation plan with owners and deadlines." Try: `hermes-the-firm:privacy-reg-gap-analysis`
> - **Sweep for policy drift** — e.g., "Look across saved PIAs, DPA reviews, and triage results to find where the privacy policy no longer matches practice." Try: `hermes-the-firm:privacy-policy-monitor`
>
...
2. **Research connector prompt.** Say:
   ...
   > "Before your first DPA review or PIA: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Hermes session: Settings → Connectors. In Hermes Agent: authorize when a skill prompts you."
...
5. **Close with the "you can change anything later" note:**
   ...
   > "Your practice profile is at `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
   >
   > - Edit the file directly for a quick change
   > - Run `hermes-the-firm:privacy-cold-start-interview --redo` for a full re-interview
   > - Run `hermes-the-firm:privacy-cold-start-interview --check-integrations` to re-check what's connected
   >
...
6. **Your practice profile learns.** End with this note:
   ...
   > - Run `hermes-the-firm:privacy-cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
   >
...
- **Don't write a DPA playbook from generic positions.** If they haven't negotiated many DPAs, say so in the config PRACTICE.md: `[POSITIONS UNTESTED — this team hasn't negotiated many DPAs yet. Treat as starting points, not settled positions.]`
