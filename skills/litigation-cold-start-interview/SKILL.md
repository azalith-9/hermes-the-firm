...
description: House cold-start for the litigation plugin — branches by role (in-house, firm associate, solo) and side (plaintiff, defense, both), captures risk calibration, landscape, and house style, and writes the practice profile PRACTICE.md. Use on a fresh install, when the user wants to set up or redo the practice profile, or to re-check available integrations.
...
1. Check `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`. If already populated and no `--redo`, ask before overwriting.
...
5. Migration: if a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/litigation-legal/*/PRACTICE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`. Date the footer.
7. Confirm with the user before finalizing: "Here's what I captured — anything wrong?"

## Flags

- `--redo` — re-run the full interview and overwrite `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` without re-running the full interview. Use after setting up a new connector (DMS, document storage, Gmail, scheduled-tasks, CLM).
...
Read `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`:
- **Does not exist** → start the interview.
- **Contains `<!-- SETUP PAUSED AT: -->`** → greet the user and offer to resume from that section.
- **Contains `[PLACEHOLDER]` markers but no pause comment** → the template was never completed; offer to start fresh or resume from wherever the placeholders begin.
- **Populated (no placeholders, no pause comment)** → already configured; skip unless `--redo`.

The template structure lives at `{plugin root}/PRACTICE.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed. If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/litigation-legal/*/PRACTICE.md` but not here, copy it forward.
...
> **`litigation-legal` is for people who work litigation — managing a portfolio of matters in-house, drafting briefs and doing discovery at a firm, or both as a solo practitioner.** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
> "Setup builds a fresh professional profile from your answers. It does not read your personal the agent history, other conversations, or your home-directory PRACTICE.md. If I notice relevant information in our conversation context — e.g., you mentioned your company or matter earlier — I'll ask before using it. Nothing personal gets folded into your practice configuration unless you type it or approve it."
...
Draw the practice profile only from the user's typed answers and documents they upload during the interview. Do not read `~/PRACTICE.md` or pull practice facts from ambient context. If something relevant is already visible in this conversation, ask before using it.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:litigation-cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:litigation-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
Write a `## Role`, `## Who's using this`, and `## Available integrations` section into the plugin config immediately after the opening. Add `## Outputs` with the work-product header rule per the PRACTICE.md template.
...
- **Demand-letter practice** — *not asked here.* Demand posture (tone, time limits, marking, signer) is set per matter, not per practice — the solo equivalent of "who signs" answers itself (you), and tone/marking/timing depend on the specific dispute. `hermes-the-firm:demand-intake` will ask when it drafts.
...
Write the completed practice profile to the plugin config, using the template at `{plugin root}/PRACTICE.md` as the section scaffold. Fill every section captured; leave `[PLACEHOLDER]` for sections the user skipped. Date the footer.
...
>
> Want to fill any of these now, or leave them as placeholders? You can also fill them later via `hermes-the-firm:litigation-cold-start-interview --redo` or by editing the plugin config directly. This one is worth thinking about before I write: [name the most important gap and why].
...
>
> - **Intake a new matter** — e.g., "Uniform intake questions, writes matter.md + history.md, appends to the portfolio log." Try: `hermes-the-firm:matter-intake`
> - **Triage an inbound demand** — e.g., "Options analysis, portfolio cross-check, handoff to matter intake if it graduates." Try: `hermes-the-firm:demand-received`
> - **Draft a demand letter** — e.g., "Privilege / FRE 408 gate, .docx output, post-send checklist, matter-creation offer." Try: `hermes-the-firm:demand-draft`
> - **Build a deposition outline** — e.g., "Docs + topics + impeachment + exhibits, tied to case theory." Try: `hermes-the-firm:deposition-prep`
> - **Issue or refresh a legal hold** — e.g., "Draft the hold memo, update the log, schedule a refresh." Try: `hermes-the-firm:legal-hold`
> - **Portfolio rollup** — e.g., "Risk distribution, upcoming deadlines, stale matters across the active portfolio." Try: `hermes-the-firm:portfolio-status`
>
...
- If `in-house`: "The in-house practice profile is now written. Every matter intake will read from it. Want to run `hermes-the-firm:matter-intake` on your most live matter to see it in action?"
...
- If `solo`: "Your solo practice profile is written — caseload shape, fee economics, how you run the office — plus the case-theory and brief-style work for a live matter. Want to run `hermes-the-firm:matter-intake` on your most live matter and see what the intake looks like with your configuration?"

### Close with the "you can change anything later" note

> "Your practice profile is at `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
> - Edit the file directly for a quick change
> - Run `hermes-the-firm:litigation-cold-start-interview --redo` for a full re-interview
> - Run `hermes-the-firm:litigation-cold-start-interview --new-matter` to reuse the practice profile on a new matter (firm-associate / solo)
> - Run `hermes-the-firm:litigation-cold-start-interview --check-integrations` to re-check what's connected
>
...
**Connect a research tool.** Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Hermes session: Settings → Connectors. In Hermes Agent: authorize when a skill prompts you.
...
- Read personal `~/PRACTICE.md` or other ambient context without asking.
