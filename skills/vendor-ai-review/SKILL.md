...
1. Read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`. Confirm vendor governance positions are populated — if not, stop and direct to setup.
...
6. AI policy consistency diff vs. `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`.
7. Output: bottom line, term-by-term, recommended redlines, if-they-won't-move routing.

```
hermes-the-firm:vendor-ai-review openai-enterprise-agreement.pdf
```
...
Read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` → `## Vendor AI governance`. Also read `## AI policy commitments`
— vendor terms can't be consistent with a use restriction our own policy imposes if
we've agreed to something different.

If `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` contains `[PLACEHOLDER]`, surface this bounce:
...
>
> **Two choices:**
> - Run `hermes-the-firm:aigov-cold-start-interview` (2 minutes) to configure your profile, then I'll review tailored to YOUR positions.
...
> "That was a generic run against default assumptions. Run `hermes-the-firm:aigov-cold-start-interview` to get output calibrated to YOUR practice — your vendor governance positions, your jurisdiction, your risk appetite. 2 minutes."
...
Review each term below. For each, extract what the vendor's contract actually says and compare it against the position in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` → `## Vendor AI governance` (standard / acceptable fallback / automatic no). The default positions come from the team's playbook, not from this skill.
...
> **Our position:** [from `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`]
> **Gap:** [specific delta — or "Aligned"]
> **Proposed fix:** [specific redline language, or "escalate — outside fallback"]

Use the severity ratings consistently (calibrated against `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` positions):
...
- 🔴 **Critical** — outside fallback; deployment should not proceed without resolution. Escalate per `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`.
...
Cross-check the vendor's terms against our AI policy commitments in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`.
...
**Before recommending signature of a vendor AI agreement (the version the company will execute):** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`. If the Role is Non-lawyer:
...
[For each 🔴 and 🟠: the fallback from `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`, or "escalate — outside fallback"
...
4. **Hosted knowledge base / RAG source** (e.g., a vector database, a third-party data corpus, a retrieval service) — the data the agent reads from
...
- It doesn't review the DPA provisions of the same agreement — run
  `hermes-the-firm:dpa-review`, if the plugin is installed, for that.
- It doesn't decide whether to accept terms outside the fallbacks. It routes those
  per the escalation table in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`.
