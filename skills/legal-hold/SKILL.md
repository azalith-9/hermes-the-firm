...
2. Otherwise: load `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/matter.md` + log row.
3. Load `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → privilege markings, hold template pointer, escalation norms.
...
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml` — log row (legal_hold fields + status)
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/matter.md` — matter context (counterparty, facts, key custodians from internal_owners)
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` — house style for litigation hold template pointer, privilege marking, escalation norms
...
> "I don't see [matter slug] in the matter log. Run `hermes-the-firm:matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't issue, refresh, or release a legal hold on a matter that hasn't been intaken — the conflicts check is the gate, and a hold issued against an unmanaged matter has no `_log.yaml` row to track `last_refresh` / `next_refresh` / `released` against."
...
**Before issuing the hold to custodians (the consequential act):** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`. If the Role is Non-lawyer:
...
**Draft the notice** to each custodian, using the house template in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` if one is configured; otherwise the default template below.
...
DATE: [effective date]
TO: [custodian name]
FROM: [signer — per `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` default]
...
**Writes:**
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/legal-hold-v1.docx` via the `docx` skill
- Appends to `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/history.md`:
  ```
...
**Writes:**
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/legal-hold-v[N].docx` (next version number)
- `history.md` entry
...
**Before releasing the hold (the consequential act — preservation obligations resume normal retention):** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md`. If the Role is Non-lawyer:
...
**Writes:**
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/[slug]/legal-hold-release.docx`
- `history.md` entry
