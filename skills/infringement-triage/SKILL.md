...
1. Read `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`. If it
   contains `[PLACEHOLDER]`, stop and direct to `hermes-the-firm:ip-cold-start-interview`.
...
8. End with recommended next steps, the non-lawyer gate if the role is
   ...
   draft the C&D via `hermes-the-firm:cease-desist` or the takedown via
   `hermes-the-firm:takedown`. Do not draft automatically.
...
```
hermes-the-firm:infringement-triage "competitor launched a tool called APEXSEED in class 9 — we have APEXLEAF registered in class 9; likely confusion?"
```

```
hermes-the-firm:infringement-triage "former engineer took notes on our model architecture to a competitor — possible trade secret?"
```

```
hermes-the-firm:infringement-triage
```
...
**Matter context.** Check `## Matter workspaces` in the practice-level PRACTICE.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `hermes-the-firm:ip-matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.hermes/plugins/config/hermes-the-firm/ip-legal/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.
...
Read `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`. Pull:
...
>
> **Two choices:**
> - Run `hermes-the-firm:ip-cold-start-interview` (2 minutes) to configure your profile, then I'll run this tailored to YOUR practice.
...
> "That was a generic run against default assumptions. Run `hermes-the-firm:ip-cold-start-interview` to get output calibrated to YOUR practice — your playbook, your jurisdiction, your risk appetite. 2 minutes."
...
**Route to `hermes-the-firm:fto-triage` for the detailed framework.** This mode is the
...
invalidity contentions, run `hermes-the-firm:claim-chart`. This triage's
...
Prepend the work-product header from `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` `## Outputs`.
...
- [routing per `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`
...
`~/.hermes/plugins/config/hermes-the-firm/ip-legal/matters/<matter-slug>/outputs/infringe-<mode>-<subject-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.hermes/plugins/config/hermes-the-firm/ip-legal/outputs/infringe-<mode>-<subject-slug>-YYYY-MM-DD.md`
...
> Want me to draft a cease-and-desist on this? Run `hermes-the-firm:cease-desist`.
...
> Want me to prepare a DMCA takedown? Run `hermes-the-firm:takedown`.
...
- **Draft the C&D, takedown, or complaint.** Those are separate skills
  (`hermes-the-firm:cease-desist`, `hermes-the-firm:takedown`) gated by the approval
