...
1. Load `~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md` → current classes, weak subjects, outline locations.
2. Apply the framework below.
3. Route by flag:
   - `--generate`: build cards from source (outline path, notes, casebook) per card-writing rules. Write to `~/.hermes/plugins/config/hermes-the-firm/law-student/flashcards/[subject]/cards.md`.
...
- `~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md` → current classes, weak subjects, existing outlines
- `~/.hermes/plugins/config/hermes-the-firm/law-student/flashcards/[subject]/cards.md` if it exists (incremental build)
...
For when the student says "let's do 5 cards on Contracts" or runs `hermes-the-firm:session Contracts 5 --flashcards`.

- Load `~/.hermes/plugins/config/hermes-the-firm/law-student/study-plan.yaml` if it exists and read `session_history` for this subject.
...
- If no `study-plan.yaml`, write to `~/.hermes/plugins/config/hermes-the-firm/law-student/session-history.yaml` instead.

### `--generate` — create cards

**Inputs:**
- Subject (class name or topic)
- Source (outline path, notes, or "use my existing outline from ~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md")
...
Per subject: total cards, bucket distribution, due today, reviewed this week. Highlight any cards that have bounced down to `new` more than twice — those are the stuck concepts worth drilling verbally via `hermes-the-firm:socratic-drill`.

## Integration with other skills

- **outline-builder:** after building or extending an outline, offer to generate flashcards from the new material
- **socratic-drill:** if a card has been wrong 2+ times, route it to `hermes-the-firm:socratic-drill` for verbal working-through — flashcards aren't enough for concepts you don't actually understand
...
- **Teach you the rule.** Cards are for drilling what you've already studied. If a card is consistently wrong, the problem is upstream — use `hermes-the-firm:socratic-drill` or re-read the source.
