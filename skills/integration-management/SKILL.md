...
5. Read/write `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code]/integration-tracker.yaml`.
...
Lives at `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code]/integration-tracker.yaml`. Read `deal-context.md` for
...
```
hermes-the-firm:integration-management --init [--deal [code]]
```

### Step 1: Load deal context

Read `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code]/deal-context.md`. If not found: ask for deal code name,
target company, close date, deal lead, and outside counsel. Write to
deal-context.md if it doesn't exist.

Read `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code]/closing-checklist.yaml` if it exists. Any items marked as
...
>   automatically from `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/deals/[code]/closing-checklist.yaml`
...
Next step: run hermes-the-firm:integration-management --contracts to import the
...
```
hermes-the-firm:integration-management --contracts [--deal [code]]
```
...
```
hermes-the-firm:integration-management --report [--deal [code]]
```
...
```
hermes-the-firm:integration-management --update [--deal [code]]
```

**Manual update:** Attorney tells the agent what changed.
...
the agent updates the relevant tracker entry, recalculates any downstream status
...
```
hermes-the-firm:integration-management --export [--format csv|table] [--section all|consents|contracts|workplan]
```
