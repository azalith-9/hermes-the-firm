...
<!--
HERMES PORT NOTE
Mechanically ported into hermes-the-firm from anthropics/claude-for-legal (plugin 'product-legal', Apache-2.0) for Hermes Agent.
...
1. Check `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md` state.
...
5. Migration: if a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/product-legal/*/PRACTICE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md` (create parent directories as needed). Show calibration table for confirmation.

## `--check-integrations`

Re-runs the integration availability check (launch tracker, document storage, Slack) and updates `## Available integrations` in `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
```
hermes-the-firm:product-cold-start-interview
```

```
hermes-the-firm:product-cold-start-interview --check-integrations
```
...
Read `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md`:
...
If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/product-legal/*/PRACTICE.md` but not at the config path, copy it forward.
...
> **`product-legal` is for people who review product launches, marketing claims, and feature risk — the legal side of shipping.** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
>
> Then: "Setup builds a fresh professional profile from your answers. It does not read your personal the agent history, other conversations, or your home-directory PRACTICE.md. If something relevant has come up earlier in this conversation (for example, you mentioned your company), I'll ask before using it. Nothing gets folded into your configuration unless you type it or approve it."
>
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:product-cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:product-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Jira isn't connected. In the agent Hermes session: Settings → Connectors → Add → Jira → sign in. In Hermes Agent: add the Jira MCP to your config or via `/mcp`. This plugin works without it — you'll paste PRDs and review docs directly — but connecting it lets the launch-watcher agent pull tickets automatically."
...
You don't need all of these. Core features work with file access alone. If you set something up later, re-run `hermes-the-firm:product-cold-start-interview --check-integrations`.
...
*Re-check: `hermes-the-firm:product-cold-start-interview --check-integrations`*
...
*Re-run: `hermes-the-firm:product-cold-start-interview --redo`*
```
...
>
> - **Legal review of a product launch** — e.g., "PRD in, review memo out against your review framework and risk calibration." Try: `hermes-the-firm:launch-review`
> - **Fast triage on a Slack question** — e.g., "'Hey legal, quick question' gets a same-minute fine / needs a real look / stop." Try: `hermes-the-firm:is-this-a-problem`
> - **Marketing claims review** — e.g., "Check copy for claims needing substantiation, comparatives, superlatives, and promises the product can't keep." Try: `hermes-the-firm:marketing-claims-review`
>
...
2. **Research connector prompt.** Say:
   ...
   > "Before your first launch review: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Hermes session: Settings → Connectors. In Hermes Agent: authorize when a skill prompts you."
...
5. **Close with the changeability note.** Say:
   ...
   > "Done. Your configuration is at `~/.hermes/plugins/config/hermes-the-firm/product-legal/PRACTICE.md` — a plain-text file you can read and edit directly. Anything you answered can be changed:
   >
   > - Edit the file directly for a quick change
   > - Run `hermes-the-firm:product-cold-start-interview --redo` for a full re-interview
   > - Run `hermes-the-firm:product-cold-start-interview --check-integrations` to re-check what's connected
   >
