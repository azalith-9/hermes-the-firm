...
description: >
  ...
  within a rolling 12-month window (configurable in `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`).
...
The gap between the playbook attorneys write and the positions they actually accept grows silently — because nobody has time to reconcile them after every deal. This agent watches the deviation log, detects when a position is being overridden consistently, and proposes a specific update to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`. The attorney approves or rejects. The playbook stays alive.
...
Both values are configurable in `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` under `## Playbook monitor settings`:
...
If these fields are absent from `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`, use the defaults above.

## What it does

### Step 1 — Read the practice profile and log

1. Read `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` in full. Extract:
...
2. Read `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/deviation-log.yaml`. Filter out:
...
If no clause crosses the threshold: log the check to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/playbook-monitor-log.yaml` and stop. Do not notify the attorney.
...
2. **Current playbook language** (exact text from `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`)
...
Current language in `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`:
...
Write all proposals to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/playbook-proposals.md`. Overwrite any existing file — stale unreviewed proposals are replaced, not accumulated.
...
*To review: run `hermes-the-firm:review-proposals`*
...
Notify the attorney via the destination in `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`:

> Playbook monitor ran — [N] proposed update(s) ready for your review.
> Run `hermes-the-firm:review-proposals` when you have a few minutes.
> Proposals: ~/.hermes/plugins/config/hermes-the-firm/commercial-legal/playbook-proposals.md

Log the run to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/playbook-monitor-log.yaml`:
...
- run_at: [ISO datetime]
  ...
  proposals_file: ~/.hermes/plugins/config/hermes-the-firm/commercial-legal/playbook-proposals.md
```

### Step 5 — Review and approval (triggered by /review-proposals command)

When the attorney runs `hermes-the-firm:review-proposals`:

1. Read `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/playbook-proposals.md`. If file doesn't exist or is empty: *"No pending proposals. Playbook is up to date."* Stop.
...
[A] Accept — apply proposed language to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`
...
```
Updating `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`:
...
5. **Reject / Defer:** log to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/playbook-monitor-log.yaml` with reason if given. Do not modify `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`. A rejected proposal is not re-raised until a new pattern emerges after the rejection date.

6. After all proposals resolved, show summary:

```
Review complete.
[N] accepted and applied to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`
[N] rejected
[N] deferred to next cycle
[N] edited and applied

`~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` last updated: [timestamp]
Next playbook check: after [N] more deals are logged
```

7. Archive: rename `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/playbook-proposals.md` to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/playbook-proposals-[YYYYMMDD].md`. The active file is now clear.

## What this agent does NOT do

- Modify `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` without explicit per-change attorney confirmation
...
Upstream ran this definition as a scheduled Claude Code agent. Hermes has no self-scheduling agents: run it on demand (or via a `cronjob` that prompts "run the playbook-monitor check"), then follow the body below.
