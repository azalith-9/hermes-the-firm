...
1. Load `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → DPA playbook. If placeholders, stop and prompt setup.
...
```
hermes-the-firm:dpa-review customer-dpa.pdf
```
...
**Matter context.** Check `## Matter workspaces` in the practice-level PRACTICE.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `hermes-the-firm:privacy-matter-workspace switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.

---

## Purpose

DPAs come in two flavors and the review is nearly opposite for each. When a customer sends their DPA, we're defending our operational flexibility. When we send one to a vendor, we're protecting our (and our customers') data. Both reviews read from the same `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` playbook but from opposite rows.

## First: which direction?

Before anything else, establish:

- **We are the processor** → customer is sending us their DPA → read `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → "When we are the processor" table
...
Before reviewing, check the outputs folder for prior work on this counterparty or processing activity. Read `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → `## Outputs` for the outputs folder path. Scan for:
...
**Carry severity from the upstream output as a floor** per the cross-skill severity floor rule in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → `## Shared guardrails`. A processing activity the triage rated 🔴 cannot be quietly downgraded to 🟢 in the DPA review; any demotion is stated and explained.
...
Read `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → `## DPA playbook`. Also read `## Privacy policy commitments` — the DPA can't contradict what the privacy policy promises.
...
Walk every DPA through these terms, clause by clause. The *specific* numeric and substantive positions (notice periods, breach timelines, acceptable/unacceptable floors) come from `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → `## DPA playbook`. The regulatory floors that any DPA has to clear come from primary law — **research the currently operative rule** for each applicable regime and cite primary sources before stating a floor.
...
Customer DPAs try to push operational burden onto us. For each clause below, compare the customer's ask to the playbook. Where the customer's ask is outside the playbook, push back to the team's standard position (from the config PRACTICE.md) and be ready to fall back to the acceptable position.
...
Prepend the work-product header from `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
[For each issue: the fallback from the config PRACTICE.md, or escalation routing if no
fallback exists]
```
...
**Before proceeding to sign or countersign a DPA (including returning an executed version, consenting to automatic execution on a counterparty platform, or instructing a signatory to execute):** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`. If the Role is Non-lawyer:
...
- It doesn't draft a DPA from scratch. If the answer is "use our template," pull the template from the seed docs path in the config PRACTICE.md.
