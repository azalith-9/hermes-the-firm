...
<!--
HERMES PORT NOTE
Mechanically ported into hermes-the-firm from anthropics/claude-for-legal (plugin 'ip-legal', Apache-2.0) by tools_dev/port_upstream.py.
Tool names, config paths, and /ip-legal:{skill} invocations were
...
Runs the cold-start interview. First run writes `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`; subsequent runs with `--redo` re-interview and show a diff before overwriting.

## Instructions

1. **Check current state:** Read `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`. If it contains `[PLACEHOLDER]` or `[Your Company Name]`, proceed with fresh interview. If populated and `--redo` not passed, ask: "Looks like you're already set up. Want to re-run the interview? This will overwrite `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` (I'll show you a diff first)."
...
5. **Migration:** If a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/ip-legal/*/PRACTICE.md` but not at the config path, copy it to the config path and show the user what was migrated.

6. **Write `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`** (create parent directories as needed) per the structure below. Use the lawyer's own words where possible.

7. **Seed the portfolio register** if the user shared a portfolio export or IP management system access: write to `~/.hermes/plugins/config/hermes-the-firm/ip-legal/portfolio.yaml`. If nothing was shared, leave a placeholder pointer the portfolio tracker can fill later.

8. **Show summary + propose next steps:**
   - "Here's what I heard — `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` is written. What did I get wrong?"
...
Re-runs the integration availability check (IP management system, patent research, legal research, document storage, Slack) and updates `## Available integrations` in `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
```
hermes-the-firm:ip-cold-start-interview
```

```
hermes-the-firm:ip-cold-start-interview --redo
```

```
hermes-the-firm:ip-cold-start-interview --check-integrations
```
...
Read `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`:
...
If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/ip-legal/*/PRACTICE.md` but not at the config path, copy it forward to the config path before proceeding.
...
> **`ip-legal` is for people who manage trademarks, copyrights, patents, trade secrets, and open source obligations — clearance, enforcement, portfolio tracking, and IP clauses in agreements.** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:ip-cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:ip-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Anaqua isn't connected. In the agent Hermes session: Settings → Connectors → Add → Anaqua → sign in. In Hermes Agent: add the Anaqua MCP to your config or via `/mcp`. This plugin works without it — portfolio lives in `portfolio.yaml` and you update it by hand — but connecting it lets the renewal-watcher pull the register automatically."
...
You don't need all of these. Core features work with file access alone. If you set something up later, re-run `hermes-the-firm:ip-cold-start-interview --check-integrations`.
...
Write the plugin config following the structure in `{plugin root}/PRACTICE.md` (the template). Use their words where you can. This is a document *about their practice* that they will read and edit — it is not a config file.
...
Write to `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` (create parent directories as needed). If the user shared a portfolio export, also seed `~/.hermes/plugins/config/hermes-the-firm/ip-legal/portfolio.yaml` with the extracted registrations.
...
>
> - **Clear a proposed trademark** — e.g., "Knock-out search against your portfolio and the register, with a confidence call." Try: `hermes-the-firm:clearance`
> - **Triage a potential infringement** — e.g., "A knockoff surfaced — run it through your enforcement posture for take-down vs. cease-and-desist vs. monitor." Try: `hermes-the-firm:infringement-triage`
> - **Freedom-to-operate analysis** — e.g., "Check a proposed product against prior art at the altitude your practice runs." Try: `hermes-the-firm:fto-triage`
> - **Draft a takedown or cease-and-desist** — e.g., "From intake to drafted letter in house voice, with escalation routing." Try: `hermes-the-firm:cease-desist`
> - **Open-source compliance check** — e.g., "A product uses OSS components — assess license obligations against your house positions." Try: `hermes-the-firm:oss-review`
> - **Portfolio renewal status** — e.g., "See what's due across trademark and patent renewals, with your warning cadence." Try: `hermes-the-firm:portfolio`
>
...
4. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your practice profile is at `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
   > - Edit the file directly for a quick change (a new approver, a revised watch list, a jurisdiction swap)
   > - Run `hermes-the-firm:ip-cold-start-interview --redo` for a full re-interview
   > - Run `hermes-the-firm:ip-cold-start-interview --check-integrations` to re-check what's connected
   >
...
5. **Before your first clearance**: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Hermes session: Settings → Connectors. In Hermes Agent: authorize when a skill prompts you.
