...
- `hermes-the-firm:takedown --send` — draft a §512(c)(3) takedown notice. Fair-use gate (*Lenz*) + loud perjury / §512(f) gate before delivery.
- `hermes-the-firm:takedown --respond` — triage a takedown someone sent you. Options: comply / counter / engage / ignore.
- `hermes-the-firm:takedown --counter` — draft a §512(g)(3) counter-notice. Loud gate for the federal-jurisdiction admission and the perjury statement.

## Instructions

1. **Read the practice profile.** Load `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`. If it contains `[PLACEHOLDER]` markers or does not exist, stop and say: "This plugin needs setup before it can give you useful output. Run `hermes-the-firm:ip-cold-start-interview` — the takedown skill depends on your approval matrix and practice profile."

2. **Check matter workspaces.** Per `## Matter workspaces`: if `Enabled` is `✗`, skip. If enabled and there is no active matter, ask: "Which matter is this for? Run `hermes-the-firm:ip-matter-workspace switch <slug>` or say `practice-level`."
...
6. **Hand off where appropriate.** `--respond` with a counter-notice recommendation chains into `hermes-the-firm:takedown --counter` — but only after the triage memo has been reviewed and the decision to counter has been made deliberately.

## Examples

```
hermes-the-firm:takedown --send
hermes-the-firm:takedown --respond ~/Downloads/youtube-takedown-notice.pdf
hermes-the-firm:takedown --counter
hermes-the-firm:takedown
```
...
- `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` → `## IP practice profile` (copyright registrations if any), `## Enforcement posture` → `Approval matrix → DMCA takedown (ordinary)` row, `## Outputs` (work-product header, role), `## Who's using this` (role — lawyer vs. non-lawyer)
- **Matter context.** Check `## Matter workspaces` in the practice-level PRACTICE.md. If `Enabled` is `✗` (in-house default), skip matter machinery. If enabled and no active matter, ask: "Which matter? Run `hermes-the-firm:ip-matter-workspace switch <slug>` or say `practice-level`." Write outputs to the active matter's folder at `~/.hermes/plugins/config/hermes-the-firm/ip-legal/matters/<matter-slug>/takedown/<slug>/` (or `takedown/<slug>/` at practice level). Never read another matter's files unless `Cross-matter context` is `on`.
...
- Next step: `hermes-the-firm:takedown --counter`
