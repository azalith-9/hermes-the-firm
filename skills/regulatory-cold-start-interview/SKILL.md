...
<!--
HERMES PORT NOTE
Mechanically ported into hermes-the-firm from anthropics/claude-for-legal (plugin 'regulatory-legal', Apache-2.0) by tools_dev/port_upstream.py.
...
1. Check `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`. If a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/regulatory-legal/*/PRACTICE.md` but not at the config path, copy it to the config path and tell the user what was migrated. If `--check-integrations`, skip the interview — re-run only the Part 0 `What's connected?` check and rewrite the `## Available integrations` table in `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`.
...
4. Write `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md` (creating parent directories as needed) with watchlist + materiality threshold.

When probing integrations: only report ✓ if an MCP tool call actually succeeded. Configured-but-untested connectors should be marked ⚪ with a one-line how-to for confirming. Never report ✓ based on `hermes mcp` configuration declarations alone — that misleads users into thinking something is wired up when it isn't.
...
Read `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`:
...
If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/regulatory-legal/*/PRACTICE.md` but not at the config path, copy it forward.
...
> **`regulatory-legal` is for people who track regulatory developments, assess policy gaps, and manage compliance obligations.** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
>
> Quick or full? (Upgrade any time with `hermes-the-firm:regulatory-cold-start-interview --full`.)

Do not read the user's home-directory `~/PRACTICE.md`, `~/user.md`, or other personal memory to pre-populate the interview. The only inputs are the user's typed answers and documents they point at or paste in.
...
- **Data sources:** setup builds a fresh professional profile from the user's answers only. It does not read the user's personal the agent history, other conversations, or home-directory PRACTICE.md. If something relevant came up earlier in this conversation (for example, the user mentioned a regulator or their sector), ask before using it. Nothing gets folded into the configuration unless the user types it or approves it.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:regulatory-cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:regulatory-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
You don't need all of these. Core features work with free feeds (Federal Register API) and file access alone. Paid feeds add enrichment; manual paste-in always works. If you set something up later, re-run `hermes-the-firm:regulatory-cold-start-interview --check-integrations`.
...
*(This feeds `hermes-the-firm:reg-feed-watcher` and the `reg-change-monitor` agent — the feed only pulls from regulators on this list. Anything not on the list is invisible to the plugin until you paste it in via `hermes-the-firm:policy-diff`.)*
...
*(This feeds `hermes-the-firm:reg-feed-watcher` and the `reg-change-monitor` agent — your materiality threshold is the filter that decides whether a new development shows up immediately, in the weekly digest, or not at all. Wrong calibration here = noisy digests you stop reading.)*
...
*(This feeds `hermes-the-firm:policy-diff` and `hermes-the-firm:gaps` — every incoming regulatory change is diffed against this library to find which policies it touches and who owns them.)*
...
**Step 4: Comment period tracking** *(This feeds `hermes-the-firm:comments` — the comment-period calendar logs deadlines and surfaces decisions when a comment window opens.)*
...
>
> - **Check regulatory feeds for what's new** — e.g., "Filtered digest of rulemaking, guidance, and enforcement against your watchlist." Try: `hermes-the-firm:reg-feed-watcher`
> - **Diff a regulatory change against your policy library** — e.g., "See exactly which internal policies a new rule impacts and what needs updating." Try: `hermes-the-firm:policy-diff`
> - **Open gaps tracker** — e.g., "What's flagged and not yet closed across your portfolio, with owner and deadline." Try: `hermes-the-firm:gaps`
> - **Track NPRM comment periods** — e.g., "What's open, comment deadlines, and a decision log on whether to file." Try: `hermes-the-firm:comments`
>
> **My suggestion for your first one:** Run `hermes-the-firm:reg-feed-watcher` — it tells you immediately whether the feeds are calibrated to your materiality threshold. Or tell me what's on your plate and I'll pick.
...
- Close with the changeability note:
  ...
  > "Done. Your configuration is at `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md` — a plain-text file you can read and edit directly. Anything you answered can be changed:
  >
  > - Edit the file directly for a quick change
  > - Run `hermes-the-firm:regulatory-cold-start-interview --redo` for a full re-interview
  > - Run `hermes-the-firm:regulatory-cold-start-interview --check-integrations` to re-check what's connected
  >
...
> - Run `hermes-the-firm:regulatory-cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
