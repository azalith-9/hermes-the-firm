...
1. Load `~/.hermes/plugins/config/hermes-the-firm/law-student/PRACTICE.md` → bar jurisdiction, exam format, bar date, weak subjects, target study hours/day, prep course.
2. Load `~/.hermes/plugins/config/hermes-the-firm/law-student/study-plan.yaml` if it exists.
...
`~/.hermes/plugins/config/hermes-the-firm/law-student/study-plan.yaml` if it exists — extend, don't overwrite.
...
Write to `~/.hermes/plugins/config/hermes-the-firm/law-student/study-plan.yaml`:
...
On the next `hermes-the-firm:study-plan --update` run (or when any skill detects the plan is stale):
...
- `hermes-the-firm:session <subject> <n>` writes results to this plan's `session_history`.
- `hermes-the-firm:bar-prep-questions` reads the plan to know which subject is scheduled for today.
- `hermes-the-firm:flashcards` can `--session <n>` and results land in the plan.
- `hermes-the-firm:socratic-drill` and `hermes-the-firm:irac-practice` session completions also append.
