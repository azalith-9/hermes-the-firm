...
1. **Follow the workflow below** and read
   `~/.hermes/plugins/config/hermes-the-firm/ip-legal/portfolio.yaml`.
...
3. **`--report [--days N]`:** Mode 2. Change the window with `--days`
   ...
   per PRACTICE.md → Outputs. Always close with the verification caveat.
...
```
hermes-the-firm:portfolio
```

```
hermes-the-firm:portfolio --report --days 180
```

```
hermes-the-firm:portfolio --add
```

```
hermes-the-firm:portfolio --update
```

```
hermes-the-firm:portfolio --audit
```
...
Lives at `~/.hermes/plugins/config/hermes-the-firm/ip-legal/portfolio.yaml`.
Structure:
...
Run hermes-the-firm:portfolio --report to see what's due.
```
...
```
hermes-the-firm:portfolio --report [--days 30|60|90|180]
```
...
Output (prepend work-product header per `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` → Outputs):
...
If the report lists more than ~10 assets, or any time the user asks: offer the dashboard (see PRACTICE.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by registration status (live / in grace / lapsed / pending), a deadline timeline, and a sortable portfolio table with jurisdiction, type, and next-action date.
...
`## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`. If the Role is **Non-lawyer**:
...
**Unwatched assets**
- Any registered marks not on the watch list in PRACTICE.md → Brand protection?
...
PRACTICE.md → Renewal alerts. If 🔴 items appear (grace / lapsed), the agent
