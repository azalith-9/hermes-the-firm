...
1. Load `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → PIA house style (trigger, structure, depth, sign-off).
...
```
hermes-the-firm:pia-generation "Location sharing feature"
```

```
hermes-the-firm:pia-generation
PRD: [Drive link]
```
...
Before writing a new PIA, check the outputs folder for prior work on the same feature, processing activity, or counterparty. Read `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → `## Outputs` for the path. Scan for:
...
**Carry severity from upstream as a floor** per the cross-skill severity floor rule in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → `## Shared guardrails`. A use-case-triage that rated the activity high-risk cannot become a PIA that concludes low-risk without stating why and what changed.
...
Read `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → `## PIA house style`. That has:
...
If the seed PIA structure is in the config PRACTICE.md, **use it**. The point is that this PIA looks like the other PIAs this team produces, not like a generic one.

## Step 0: Is a PIA needed?

Check the trigger criteria in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`. That is the team's house answer.
...
**Use the seed PIA structure from the config PRACTICE.md.** If none was captured, use this default. Prepend the work-product header from `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
|---|---|---|
| [commitment from config PRACTICE.md privacy policy section] | 🟢 / 🟡 | |
...
Every PIA should cross-check against the privacy policy commitments in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`. The common drift:
...
- **To the sign-off process:** Per `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md` → who approves PIAs.
...
**Before proceeding to submit a DPIA (or any equivalent impact assessment) to a regulator, supervisory authority, or enforcement body:** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/privacy-legal/PRACTICE.md`. If the Role is Non-lawyer:
