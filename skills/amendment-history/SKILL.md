...
```
hermes-the-firm:amendment-history acme-msa.pdf amendment-1.pdf amendment-2.pdf
```

```
hermes-the-firm:amendment-history --provision indemnity
```

```
hermes-the-firm:amendment-history
...
This skill reads the base agreement and amendments — often privileged or confidential in their own right, and typically used for privileged analysis. The output inherits the source's privilege and confidentiality status. Prepend the work-product header from `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` `## Outputs` to every output below, distribute only within the privilege circle, and store it where privileged materials live. Strip the header before any external delivery.
...
- It does not compare against the playbook in `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` — that is the
