---
name: clinic-cold-start-interview
description: >
  ...
  handbook/rules upload. Writes PRACTICE.md so every other skill and every
  student who runs /ramp reads from the same clinic context. Use on fresh
  install, when PRACTICE.md has placeholders, when re-doing setup with --redo,
...
1. Check `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md`. If populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/legal-clinic/*/PRACTICE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` including `## Who's using this` and `## Available integrations`. Show supervision choice and practice-area templates for confirmation.
7. Offer `hermes-the-firm:ramp` preview.

```
hermes-the-firm:clinic-cold-start-interview
```

**`--check-integrations`:** Re-run only the Part 0 integration-availability check (Clio, document storage). Updates `## Available integrations` in `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` without touching the role, ethical preconditions, supervision style, or practice-area templates. Use after adding or removing an MCP connector.
...
Read `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md`:
...
> **`legal-clinic` is for supervising attorneys setting up a law school clinic and onboarding students.** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
- **Pause and resume.** Tell the supervising attorney up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:clinic-cold-start-interview` again later and I'll pick up where you left off." When the attorney pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:clinic-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the attorney: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> 2. **No, I'm a student / staff / administrator.** Stop. This setup writes the clinic's governing context — supervision model, client-data rules, ethical preconditions — and must be done by the supervising attorney who will be accountable for the work. Ask them to run `hermes-the-firm:clinic-cold-start-interview`. Students run `hermes-the-firm:ramp` to onboard each semester.
...
1. **Account tier and data-handling terms.** Your the agent account tier and its data retention and training policies — Team, Enterprise, Work, Education, and individual accounts have different guarantees about retention, use for training, and subprocessor handling. Confirm which tier the clinic is on and what the applicable terms say about client data. Document the answer in the plugin config.
...
Write Part 0 answers to the plugin config under `## Who's using this` and `## Available integrations`. If a populated PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/legal-clinic/*/PRACTICE.md` but not here, copy it forward first.
...
> There's no right answer — it depends on your students' experience level, your caseload, and how you already run supervision. You can change this later by editing PRACTICE.md.
...
>
> You can set this per document type later with `hermes-the-firm:build-guide`. For now, pick a default.
...
> Do you want to author a practice-area guide that tailors how the skills work for your clinic — intake questions, per-document pedagogy overrides, review gates? I can help you build one in 5-10 minutes with `hermes-the-firm:build-guide`. You can also do it later. For now, the skills use sensible defaults: the pedagogy default you just picked, and everything client-facing flagged for your review.

Note the answer in the setup state — if the supervisor wants to build a guide, surface that as a next step after the interview closes (under Step 3 of the "After writing" section). Do not interrupt this interview to run `hermes-the-firm:build-guide` inline; finish the profile first, then offer the handoff.
...
Per the PRACTICE.md template. Key sections:
...
**LIMITED DATA flag:** if fewer than 10 materials were shared across the interview, add a `> LIMITED DATA` note at the top of PRACTICE.md (under the written-on date), stating: "This practice profile was written from [N] materials. Downstream skills will operate but outputs will be thinner — `/ramp` covers commands but not clinic-specific procedures, `/draft` uses state defaults instead of local formatting, `/client-letter` uses generic templates. Re-run `hermes-the-firm:clinic-cold-start-interview --redo` after collecting more exemplars to sharpen calibration."
...
>
> - **Student intake on a new case** — e.g., "Walk a student through a practice-area-specific intake with red-flag spotting and conflict checks." Try: `hermes-the-firm:client-intake`
> - **Draft a client letter at 6th-grade reading level** — e.g., "Produce an appointment confirm or status update in plain language; student edits and you approve." Try: `hermes-the-firm:client-letter`
> - **Build an IRAC memo scaffold** — e.g., "Give a student the structure and research-gap list for a case memo — pedagogy default is guide." Try: `hermes-the-firm:memo`
> - **Track deadlines across the active docket** — e.g., "See what's due in the next 14 / 7 / 3 / 1 days with warnings per your cadence." Try: `hermes-the-firm:deadlines`
> - **Ramp up a new cohort** — e.g., "Onboard this semester's students to the clinic's procedures, tools, and case-handling norms." Try: `hermes-the-firm:ramp`
> - **Semester handoff** — e.g., "Build per-case transition memos for the incoming cohort." Try: `hermes-the-firm:semester-handoff`
>
...
> Done. Your clinic's configuration is at `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
> - Edit the file directly for a quick change
> - Run `hermes-the-firm:clinic-cold-start-interview --redo` for a full re-interview
> - Run `hermes-the-firm:clinic-cold-start-interview --check-integrations` to re-check what's connected
>
...
- **Replace the clinic's existing case management.** If the clinic uses Clio, this plugin works alongside it (Clio MCP is an open integration question — see `hermes mcp` configuration).
