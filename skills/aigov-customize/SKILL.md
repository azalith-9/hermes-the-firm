...
The user typed `hermes-the-firm:aigov-customize`. They want to change something
...
1. **Read the config.** Read
   ...
   (and `~/.hermes/plugins/config/hermes-the-firm/company-profile.md` one
   level up). If the plugin config does not exist or still contains
   `[PLACEHOLDER]` values, say:

   > You haven't run setup yet. Run `hermes-the-firm:aigov-cold-start-interview`
...
2. **Show the customizable map.** List what's in the profile, grouped, with a
   ...
   - **AI system inventory** — per-system role (provider / deployer / etc.) and
     tier under the EU AI Act. Run `hermes-the-firm:ai-inventory` for
...
5. **For shared-profile changes** (company name, industry, jurisdictions,
   ...
   `~/.hermes/plugins/config/hermes-the-firm/company-profile.md` and note:
...
6. **Close.**
   ...
   > run `hermes-the-firm:aigov-customize` anytime.
