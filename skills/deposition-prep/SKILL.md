...
1. Load `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → case theory, key facts.
...
Two rules that govern every citation and every quotation pulled from the record into this outline. Canonical statement lives in the plugin's `PRACTICE.md` shared guardrails; repeated here because an impeachment confrontation built on a misquoted prior statement or a misgrounded transcript cite collapses the impeachment.
...
`~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → case theory (theory, pivot fact, key facts for/against), eDiscovery platform.

**Conflicts gate — unbypassable.** Before building an outline, check `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:

> "I don't see [matter slug] in the matter log. Run `hermes-the-firm:matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't build a deposition outline on a matter that hasn't been intaken — the conflicts check is the gate."
...
- Facts from `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → key facts for us, that this witness can establish
