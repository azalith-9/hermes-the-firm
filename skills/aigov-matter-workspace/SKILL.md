...
- `hermes-the-firm:aigov-matter-workspace new <slug>` — create a new matter workspace, run a short intake, write `matter.md`
- `hermes-the-firm:aigov-matter-workspace list` — list matters with status and active flag
- `hermes-the-firm:aigov-matter-workspace switch <slug>` — set the active matter
- `hermes-the-firm:aigov-matter-workspace close <slug>` — archive a matter (move to `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/matters/_archived/`, never delete)
- `hermes-the-firm:aigov-matter-workspace none` — detach from any active matter, work at practice-level only

## Instructions

1. Read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` — confirm the `## Matter workspaces` section is populated. If `Enabled` is `✗`, tell the user: "Matter workspaces are off — you're configured as an in-house practice with one client, so the plugin works from practice-level context automatically. If you actually work across multiple clients, re-run `hermes-the-firm:aigov-cold-start-interview --redo` and select a private-practice setting. Otherwise, you don't need `/matter-workspace` at all." Don't error — the disabled state is the expected one for in-house users.
2. Use the workflow below.
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level PRACTICE.md.
   - `close` → move `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/matters/<slug>/` to `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/matters/_archived/<slug>/`, log the close date in `history.md`.
   - `none` → set `Active matter:` to `none — practice-level context only`.
4. Show the user what changed and confirm before writing.

## Notes

- The skill never reads across matters unless `Cross-matter context` is `on` in the practice-level PRACTICE.md.
...
**Default state is off.** In-house users never see this — they run at practice-level only. Matter workspaces turn on at cold-start for private-practice users, or by editing `## Matter workspaces` in the practice-level PRACTICE.md. If `Enabled` is `✗`, this skill does not run; the workflow above explains the disabled state and suggests `hermes-the-firm:aigov-cold-start-interview --redo` for users who actually need matter isolation.
...
```
~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/
├── PRACTICE.md                       # practice-level practice profile
└── matters/
    ├── <slug>/
...
## Active matter is in the practice PRACTICE.md

The `Active matter:` line under `## Matter workspaces` in the practice-level PRACTICE.md is the single source of truth. Switching a matter edits that line. No separate state file.
...
6. Do **not** auto-switch to the new matter. Ask: "Want to switch to `<slug>` now? (`hermes-the-firm:aigov-matter-workspace switch <slug>`)"
...
1. Confirm `matters/<slug>/matter.md` exists. If not, offer `hermes-the-firm:aigov-matter-workspace new <slug>`.
2. Edit the `Active matter:` line in the practice-level PRACTICE.md to `Active matter: <slug>`.
...
Set `Active matter:` in the practice-level PRACTICE.md to `none — practice-level context only`. Confirm with the user.

## `matter.md` template

```markdown
[WORK-PRODUCT HEADER — per plugin config ## Outputs — differs by role; see `## Who's using this` in the practice-level PRACTICE.md]
...
The practice-level PRACTICE.md has a `Cross-matter context:` flag. When it's `off` (the default), a skill working in matter A **never reads** files in `matters/B/` for any other `B`. Period. This is the confidentiality guarantee the setting exists to provide.
