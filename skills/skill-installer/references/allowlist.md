...
```
~/.hermes/plugins/config/hermes-the-firm/legal-builder-hub/allowlist.yaml
```
...
administrator-controlled file that the agent reads before any analysis runs and
whose enforcement does not depend on the agent correctly analyzing the skill.
...
connectors:
  # MCP server URLs a community skill may reference in its `hermes mcp` configuration.
