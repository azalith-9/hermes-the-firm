...
1. Read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`. If the practice profile is unpopulated, stop and direct to `hermes-the-firm:aigov-cold-start-interview`.
...
```
hermes-the-firm:policy-starter
hermes-the-firm:policy-starter "we need an AI policy for our 30-lawyer firm"
hermes-the-firm:policy-starter "update our existing policy for the 2026 state AI laws"
```
...
up doing with Copilot and the agent for Work. This skill produces a **draft** policy
...
3. **Training materials** — once the policy is adopted, `hermes-the-firm:aia-generation` can be used to produce per-use-case training notes.
4. **Vendor sweep** — once the policy is adopted, `hermes-the-firm:vendor-ai-review` should be run against the vendors the policy references to check conformance.
5. **Gap check against new regulation** — pair with `hermes-the-firm:aigov-reg-gap-analysis` to test the draft against a specific regulation or guidance before adoption.
