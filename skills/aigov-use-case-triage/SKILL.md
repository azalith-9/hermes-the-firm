...
1. Read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`. Confirm registry is populated — if not, stop and direct to setup.
...
```
hermes-the-firm:aigov-use-case-triage "Sales team wants to score leads with AI automatically"
```
...
## Read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` first

Before triaging, always read `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`. The use case registry and red lines there
...
>
> **Two choices:**
> - Run `hermes-the-firm:aigov-cold-start-interview` (2 minutes) to configure your profile, then I'll triage tailored to YOUR practice.
...
> "That was a generic run against default assumptions. Run `hermes-the-firm:aigov-cold-start-interview` to get output calibrated to YOUR practice — your registry, your jurisdiction, your risk appetite. 2 minutes."

**Jurisdictional scope.** Triage applies the registry, red lines, and governance tiers configured for the regulatory footprint in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`. AI rules vary materially by jurisdiction — an APPROVED classification in one footprint may be CONDITIONAL or prohibited in another. If deployment touches a jurisdiction not in the footprint, surface that and re-triage rather than extending by analogy.
...
Check the use case registry in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` for a direct or close match.
...
The APPROVED / CONDITIONAL / NOT APPROVED buckets, the red-line definitions, and the CONDITIONAL required-controls list all come from `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` → `## AI use case triage criteria` and `## Use case registry`. If the playbook doesn't define a criterion the use case turns on, ask the user: "Your playbook doesn't cover [specific question]. What's your default position? I'll add it to `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` so the next triage is consistent."

**Before issuing an APPROVED classification (approving an AI use case for deployment):** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`. If the Role is Non-lawyer:
...
**Governance tier:** [Standard / Elevated / High — per `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md`]
...
`hermes-the-firm:aia-generation [use case]`
...
> an AI impact assessment. Use `hermes-the-firm:pia-generation [use case]`, if the
...
> Use `hermes-the-firm:launch-review`, if the plugin is installed — it will detect
...
> "Add to `~/.hermes/plugins/config/hermes-the-firm/ai-governance-legal/PRACTICE.md` → Use case registry. This means next time the same request
