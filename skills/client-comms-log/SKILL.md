...
3. Route by flag:
   - `--add` (default): capture direction, medium, student, summary, action items, follow-up due. Confirm with user. Append (prepend most-recent-first) to `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/client-comms/[case-id]/log.md`.
...
4. Integration: offer `hermes-the-firm:deadlines --add` if the log establishes a deadline; route to `hermes-the-firm:semester-handoff` via `--summary` when relevant.
...
- `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/client-comms/[case-id]/log.md` (if exists) — append target
- `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` → not heavily read; this skill is case-scoped
...
**Append** to `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/client-comms/[case-id]/log.md`. If the log doesn't exist, create it with a header:
