...
1. Load `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md` → policy library index.
...
`~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md` → policy library index (policies, locations, owners).
...
- Note the comment deadline and the team's comment-decision owner from `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`.
...
[other-policy-1] and [other-policy-2] — rerun `hermes-the-firm:policy-diff` against those.
...
This skill reads the policy library index from `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`. When the index is empty or still `[PLACEHOLDER]`:

- **Policy library empty:** flag every requirement as "no policy match" by default and append to the output: "The policy library in your configuration is empty, so every requirement is flagged as a new-policy gap. If you have policies that address these requirements, add them to the library with `hermes-the-firm:regulatory-cold-start-interview --redo` or by editing `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md`, then re-run the diff."
- **Owner missing for a matched policy:** leave the Owner cell blank in the summary and append: "Policy owners aren't set for [list]. Assign them with `hermes-the-firm:regulatory-cold-start-interview --redo` or by editing the policy library in `~/.hermes/plugins/config/hermes-the-firm/regulatory-legal/PRACTICE.md` so gap-surfacer can route."
