...
Reviews the IP clauses in an agreement against the practice profile in `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`. Flags assignment gaps, ownership ambiguity, license-scope issues, and IP warranty/indemnity problems. Produces a memo with per-clause findings, prioritized by risk, with suggested redline language where appropriate.

## Instructions

1. **Load `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`.** If placeholders present, stop and prompt: "Run `hermes-the-firm:ip-cold-start-interview` first — I need to learn your practice profile before I can review IP clauses against it."
...
```
hermes-the-firm:ip-clause-review ~/Documents/vendor-sow.pdf
hermes-the-firm:ip-clause-review https://docs.google.com/document/d/...
hermes-the-firm:ip-clause-review
```
...
**Before reading the agreement, read `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`.** If it is missing or still contains placeholders, stop and run `hermes-the-firm:ip-cold-start-interview`. The practice profile tells you:
...
**Escalation:** Per `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md`, assignment-scope gaps escalate to [approver].
```
...
Prepend the work-product header from `~/.hermes/plugins/config/hermes-the-firm/ip-legal/PRACTICE.md` → `## Outputs` (it differs by user role — see `## Who's using this`).
