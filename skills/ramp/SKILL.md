...
1. Check `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` is set up. If placeholders: "Ask [professor] to run `hermes-the-firm:clinic-cold-start-interview` first."
...
```
hermes-the-firm:ramp
```

```
hermes-the-firm:ramp --card
```
...
`~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` → clinic profile, practice areas, jurisdiction, handbook path, supervision style, practice-area templates.
...
Read from `~/.hermes/plugins/config/hermes-the-firm/legal-clinic/PRACTICE.md` and the ingested handbook. Cover, interactively:
...
- **Supervision** — how review works in this clinic (per the supervision style in PRACTICE.md). Be specific: "Before anything goes to a client or a court, [it goes in the review queue / you check with Professor X / etc.]"
...
- **[Supervision reminder per PRACTICE.md style]** — what gets reviewed before it goes out, and how.
...
- What the agent can help with / what it can't (starting points yes, final work product no, authoritative citations no)
- Verification habits (the bullets from Part 4)
- Who to ask when stuck (professor name from PRACTICE.md)
