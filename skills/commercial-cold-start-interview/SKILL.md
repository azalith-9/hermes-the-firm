...
description: >
  ...
  `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` is missing or still contains template
...
<!--
HERMES PORT NOTE
Mechanically ported into hermes-the-firm from anthropics/claude-for-legal (plugin 'commercial-legal', Apache-2.0) by tools_dev/port_upstream.py.
...
Runs the cold-start interview. First run writes `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`; subsequent runs with `--redo` re-interview and show a diff before overwriting.

## Instructions

1. **Check current state:** Read `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`. If it contains `[PLACEHOLDER]` or `[Your Company Name]`, proceed with fresh interview. If populated and `--redo` not passed, ask: "Looks like you're already set up. Want to re-run the interview? This will overwrite `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` (I'll show you a diff first)."
...
5. **Migration:** If a populated PRACTICE.md (no `[PLACEHOLDER]` markers) exists at `~/.hermes/plugins/cache/hermes-the-firm/commercial-legal/*/PRACTICE.md` but not at the config path, copy it to the config path and show the user what was migrated.

6. **Write `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`** (create parent directories as needed) per the structure below. Use the lawyer's own words where possible.

7. **Show summary + propose next steps:**
   - "Here's what I heard — `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` is written. What did I get wrong?"
   - Offer a test review: "Want to throw a contract at me?"
   - If a [CLM] is connected: offer to bulk-load the renewal register

## `--check-integrations`

Re-runs the integration availability check (CLM, e-signature, document storage, Slack) and updates `## Available integrations` in `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.

When probing: only report ✓ if an MCP tool call actually succeeded. Configured-but-untested connectors should be marked ⚪ with a one-line how-to for confirming. Never report ✓ based on `hermes mcp` configuration declarations alone — that misleads users into thinking something is wired up when it isn't.

## `--side sales` / `--side purchasing`

Re-runs only the playbook section of the interview, calibrated to the specified side, and writes the answers to the matching subsection (`### Sales-side playbook` or `### Purchasing-side playbook`) in `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`. Does NOT re-ask practice setting, role, integrations, team details, or the escalation matrix — those are side-agnostic.
...
```
hermes-the-firm:commercial-cold-start-interview
```

```
hermes-the-firm:commercial-cold-start-interview --redo
```

```
hermes-the-firm:commercial-cold-start-interview --check-integrations
```

```
hermes-the-firm:commercial-cold-start-interview --side purchasing
```
...
Read `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md`:
...
The template structure lives at `{plugin root}/PRACTICE.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed.

If a PRACTICE.md exists at the old cache path `~/.hermes/plugins/cache/hermes-the-firm/commercial-legal/*/PRACTICE.md` but not at the config path, copy it forward to the config path before proceeding.
...
Look for `~/.hermes/plugins/config/hermes-the-firm/company-profile.md`.
...
> **`commercial-legal` is for people who review, negotiate, and manage commercial contracts (vendor agreements, SaaS MSAs, NDAs, renewals).** Not your area? `hermes-the-firm:related-skills-surfacer`.
>
...
>
> Quick or full? (Upgrade any time with `hermes-the-firm:commercial-cold-start-interview --full`.)
...
**Fresh professional profile.** Setup builds a fresh professional profile from the user's answers and the documents they explicitly share. It does not read the user's personal the agent history, unrelated conversations, or their home-directory PRACTICE.md. If something relevant surfaces in the current conversation context (e.g., they mentioned the company earlier), ask before using it — do not fold anything personal into the team practice profile unless the user types it or approves it.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `hermes-the-firm:commercial-cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` with a `<!-- SETUP PAUSED AT: [section name] — run hermes-the-firm:commercial-cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.

**Verify user-stated legal facts as they come up in setup.** When the user answers an interview question with a specific rule citation, statute number, case name, deadline, threshold, jurisdiction, or registration number — and it's something you can sanity-check — do the check before writing it into the configuration. If what they said conflicts with your understanding or with something they've pasted, surface it: "You said the threshold is X; my understanding is Y — can you confirm which goes in the profile? `[premise flagged — verify]`" A wrong fact written into PRACTICE.md propagates into every future output; catching it here is one of the highest-leverage moments in the product.
...
**Check what's actually connected, not what's configured.** A connector listed in `hermes mcp` configuration is *available*. A connector that's actually responding is *connected*. These are different, and confusing them destroys trust. For each connector this plugin uses:
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Box isn't connected. In the agent Hermes session: Settings → Connectors → Add → Box → sign in. In Hermes Agent: add the Box MCP to your config or via `/mcp`. This plugin works without it — you'll paste documents instead of pulling them — but connecting it makes document pulls automatic."

Then report findings in this form:

> - ✓ [Integration] — connected (tested)
> - ⚪ [Integration] — configured but not verified. Open your MCP settings to confirm.
> - ✗ [Integration] — not found. [Feature] will fall back to [manual alternative]. [How to connect.] If you set this up later, re-run `hermes-the-firm:commercial-cold-start-interview --check-integrations`.
>
...
If the user picked **both**, run Part 2 once for sales-side now. Tell them: "We'll come back to purchasing-side with `hermes-the-firm:commercial-cold-start-interview --side purchasing` when we're done here." Write sales-side answers to `### Sales-side playbook`.
...
*Re-check: `hermes-the-firm:commercial-cold-start-interview --check-integrations`*
...
*[If not configured yet: leave the pointer "[Not configured — run hermes-the-firm:commercial-cold-start-interview --side sales to build it]" in place of the subsections below.]*
...
*[If not configured yet: leave the pointer "[Not configured — run hermes-the-firm:commercial-cold-start-interview --side purchasing to build it]" in place of the subsections below.]*
...
*To re-run the interview: `hermes-the-firm:commercial-cold-start-interview --redo`*
```
...
>
> - **Review a vendor MSA against your playbook** — e.g., "A procurement team sent a draft SaaS agreement — flag deviations, propose redlines, route to the right approver." Try: `hermes-the-firm:review`
> - **Triage an inbound NDA to GREEN / YELLOW / RED** — e.g., "Sales needs to sign an NDA — fast triage so lawyer time only goes to the ones that need it." Try: `hermes-the-firm:review`
> - **Track renewal deadlines** — e.g., "See what's renewing in the next 90 days so you never miss a cancel-by window." Try: `hermes-the-firm:renewal-tracker`
> - **Trace a clause across amendments** — e.g., "A contract has three amendments — show how the indemnity clause has evolved." Try: `hermes-the-firm:amendment-history`
> - **Escalate a deviation** — e.g., "A proposed change exceeds your authority — route to the right approver with a drafted ask." Try: `hermes-the-firm:escalation-flagger`
> - **Review pending playbook updates** — e.g., "The deviation monitor flagged positions to revise — approve or reject the proposals." Try: `hermes-the-firm:review-proposals`
>
...
2. **Research connector prompt.** Say:
   ...
   > "Before your first contract review: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Hermes session: Settings → Connectors. In Hermes Agent: authorize when a skill prompts you."
...
5. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your practice profile is at `~/.hermes/plugins/config/hermes-the-firm/commercial-legal/PRACTICE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
   > - Edit the file directly for a quick change (a new fallback, a revised threshold, a name swap)
   > - Run `hermes-the-firm:commercial-cold-start-interview --redo` for a full re-interview
   > - Run `hermes-the-firm:commercial-cold-start-interview --check-integrations` to re-check what's connected
   >
...
> - Run `hermes-the-firm:commercial-cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
