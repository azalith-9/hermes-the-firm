...
1. Load `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → risk calibration (defines how to read the `risk:` field).
2. Follow the workflow and reference below.
3. Parse `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml`. Filter closed matters by default (include with `--all`).
...
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml` — source of truth
- `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` — risk calibration (to interpret risk/materiality fields correctly)
...
If the portfolio has more than ~10 matters, or any time the user asks: offer the dashboard (see PRACTICE.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by risk tier, a timeline of upcoming deadlines, and a sortable matter ledger with status, conflicts check, and last-touched date.
