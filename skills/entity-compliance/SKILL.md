...
1. Load `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` → `## Entity Management` (entity table, jurisdictions, registered agent).
...
3. Read/write `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/entities/compliance-tracker.yaml`.
...
is a file you own, the agent updates it on command, and you export it when you need
to share it.
...
Lives at `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/entities/compliance-tracker.yaml`. Structure:
...
Read `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` → `## Entity Management` → Entity table. If the entity table
...
Generate `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/entities/compliance-tracker.yaml` with all entities and their
...
Run hermes-the-firm:entity-compliance --report to see what's due.
```
...
```
hermes-the-firm:entity-compliance --report [--days 30|60|90|180]
```
...
If the tracker covers more than ~10 entities, or any time the user asks: offer the dashboard (see PRACTICE.md `## Outputs → Dashboard offer for data-heavy outputs`). Shape the offer for this output — counts by filing status (overdue / due soon / filed / unknown), counts by good-standing state, and a sortable entity table with jurisdiction, filing type, and next due date.
...
**Before directing or confirming a filing:** Read `## Who's using this` in `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`. If the Role is **Non-lawyer**:
...
```
hermes-the-firm:entity-compliance --update
```

Attorney tells the agent what was filed:
...
```
hermes-the-firm:entity-compliance --update --from-report
```

User uploads a CT Corp, National Registered Agents, or similar compliance
report (PDF, CSV, or Excel). the agent reads it and updates matching entities:
...
```
hermes-the-firm:entity-compliance --sweep
```
...
```
hermes-the-firm:entity-compliance --audit
```
...
- Based on `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` entity table: are there states in the company's
  ...
  the agent can flag the question but cannot determine presence independently.

**Intercompany agreement gaps:**
- From `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`: if intercompany agreements are marked as partial or no,
...
POTENTIAL GAPS
  Foreign qualification: [flag question — confirm operational presence in:]
    [list of states from `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md` footprint not in tracker as qualified]
  Intercompany agreements: [status from `~/.hermes/plugins/config/hermes-the-firm/corporate-legal/PRACTICE.md`]
...
```
hermes-the-firm:entity-compliance --export [--format csv|table]
```
