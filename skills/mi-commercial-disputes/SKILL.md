...
See `mi-law-references` for the model UCC and the corpus
(`us-statute-lookup`) for Michigan's enacting sections (MCL 440.9).
...
- `mi-law-references` — Michigan Compiled Laws, Michigan Court
  Rules, Michigan Rules of Evidence, and the federal/UCC layer (served
  by the open-us-law corpus via `us-statute-lookup`)
