...
Per `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` → Landscape → Frequent fora and the per-matter cadence in `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml`.
...
1. Read `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/PRACTICE.md` for house style, escalation rules, and the frequent-fora list. Read `~/.hermes/plugins/config/hermes-the-firm/litigation-legal/matters/_log.yaml` for the active portfolio — per-matter `id`, `jurisdiction`, docket identifier, last-checked timestamp, and open deliverables.
...
5. Write `./out/docket-report-<date>.md` with per-matter sections and a machine-readable `./out/deadlines.yaml` the docketing system can ingest. Update each matter's `history.md` with a dated entry noting what was pulled. Post a summary to Slack per the escalation channel in PRACTICE.md.
...
Upstream ran this definition as a scheduled Claude Code agent. Hermes has no self-scheduling agents: run it on demand (or via a `cronjob` that prompts "run the docket-watcher check"), then follow the body below.
